...
当前版本：**V2.2**（121 条审核规则 / 9 大分类 / 3 级严重级别；银行级改造：
...
| DDL 规范 | DDL | 23 | 建表语句：主键、引擎、字符集、字段类型 |
| DML 规范 | DML | 9 | 增删改查：SELECT*、无 WHERE、子查询深度 |
| 索引规范 | Index | 10 | 索引数量、冗余索引、前缀索引 |
| 分布式规范 | Distributed | 15 | 分片键查询/更新/建表声明(R077)、跨分片操作 |
...
| 规则库 | `backend/engine/rules/` | 121 条审核规则的实现 |
