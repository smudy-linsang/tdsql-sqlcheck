...
cp "$ROOT_DIR/docs/TEST-PLAN-v${VERSION}-内网智能体大库测试方案与操作指引.md" "$STAGE_DIR/docs/" 2>/dev/null || true
...
cp "$ROOT_DIR/docs/DETAIL-v${VERSION}-大库在线元数据审核稳定性修复.md" "$STAGE_DIR/docs/" 2>/dev/null || true
cp "$ROOT_DIR/docs/REPORT-6000表元数据审核失败根因深度排查与解决方案报告.md" "$STAGE_DIR/docs/" 2>/dev/null || true
