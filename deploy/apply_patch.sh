...
[ -f "$TARGET_DIR/backend/services/report_service.py" ] && cp "$TARGET_DIR/backend/services/report_service.py" "$BACKUP_DIR/backend/services/"
...
[ -f "$SCRIPT_DIR/backend/services/report_service.py" ] && cp "$SCRIPT_DIR/backend/services/report_service.py" "$TARGET_DIR/backend/services/report_service.py"
...
echo "[4/4] 验证服务状态..."
sleep 2
NEW_VER=$(cat "$TARGET_DIR/VERSION" 2>/dev/null || echo "unknown")
echo "✅ 补丁应用成功！当前版本已更新为: $NEW_VER"
