...
while [[ $# -gt 0 ]]; do case "$1" in
  --port) PORT="$2"; shift 2;; --host) HOST="$2"; shift 2;; *) shift;; esac; done
BASE="http://${HOST}:${PORT}"
PASS=0; FAILC=0
...
echo "════ 部署验证 v1.0.3 @ ${BASE} ════"

# 1. 健康检查与版本
HV=$(curl -s -m 5 "${BASE}/health")
...
# 2. 前端资产
curl -s -m 5 "${BASE}/" | grep -q "TDSQL" && ok "首页可访问" || bad "首页不可访问"
for f in /static/js/app.js /static/css/app.css /static/vendor/vue.global.prod.js; do
  CODE=$(curl -s -o /dev/null -w "%{http_code}" -m 5 "${BASE}${f}")
...
TOKEN=""
if [[ -n "$PW" ]]; then
  LOGIN=$(curl -s -m 5 -XPOST "${BASE}/api/v1/auth/login" -H 'Content-Type: application/json' \
...
RULES=$(curl -s -m 5 "${AUTHH[@]}" "${BASE}/api/v1/rules")
...
AUD=$(curl -s -m 10 -XPOST "${AUTHH[@]}" "${BASE}/api/v1/audit/sql" \
...
DASH=$(curl -s -m 5 "${AUTHH[@]}" "${BASE}/api/v1/dashboard/summary")
echo "$DASH" | J 'd["audit"]["today_count"]' >/dev/null && ok "元数据库读写正常(概览)" || bad "概览接口异常（检查TDSQL元数据库连接）"

# 7. Prometheus 指标
curl -s -m 5 "${BASE}/metrics" | grep -q "tdsql_" && ok "/metrics 指标输出" || bad "/metrics 无输出（确认 METRICS_ENABLED=true）"
