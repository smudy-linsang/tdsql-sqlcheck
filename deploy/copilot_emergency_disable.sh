...
# 回收 runner 的文本复核子进程（仅本模块固定模块名，不波及 metadata-runner）
pkill -f "backend.workers.copilot_text_worker" 2>/dev/null || true

# ── 4. 最小本地事件记录（无正文/无敏感值）──────────────────────────────────
EVENT_LOG="${INSTALL_DIR}/logs/copilot-emergency.log"
mkdir -p "${INSTALL_DIR}/logs" 2>/dev/null || true
printf '%s incident=%s db_side=%s unit=%s result=disabled\n' \
  "${TS}" "${INCIDENT_ID}" "${DB_OK}" "${UNIT}" >> "${EVENT_LOG}" 2>/dev/null || true
