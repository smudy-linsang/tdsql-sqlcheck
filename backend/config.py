...
APP_VERSION = "1.6.3.0"
APP_DESCRIPTION = "银行级SQL质量管控与慢SQL分析平台（V1.6.3.0 - 深度诊断新增G14表类型统计：分布式按原厂三条proxy命令口径统计分片/广播/单表，集中式按BASE TABLE口径；二级分区物理子表剔除并单列；information_schema交叉校验；统计结果留档可回看）"
