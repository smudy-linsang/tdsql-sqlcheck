-- v1.6.4.0 / CP-1 Rev.D：AI Copilot B组（助手模块组）——九张业务表
-- 设计出处：DETAIL §10.1/§10.2（B-01 方案乙）
-- version_key: copilot_v1_001_business（独立编号空间，共享 schema_migrations 台账）
--
-- B组契约（方案乙）：
--   · 独立目录 backend/copilot_schema 发现，核心 loader 不加载本文件；
--   · 专属维护入口 python -m backend.services.copilot.schema --apply 执行，
--     不在 Web 启动/import 期跑 DDL；失败仅助手 UNAVAILABLE，不阻断原功能；
--   · 完整结构验收（表存在/引擎/排序规则/列全集/类型/可空/默认/主键/索引）；
--   · 本组 DDL 只允许下列九表，不增加对既有业务表的写接点。
...
CREATE TABLE IF NOT EXISTS copilot_providers (
  ...
  COMMENT='v1.6.4.0 Copilot 模型提供方配置（B组）';
...
CREATE TABLE IF NOT EXISTS copilot_scene_routes (
  ...
  COMMENT='v1.6.4.0 Copilot 场景路由（B组）；空主 provider 即本地模式';
...
CREATE TABLE IF NOT EXISTS copilot_instance_grants (
  ...
  COMMENT='v1.6.4.0 Copilot 实例资料授权（B组，申请/复核分离）';
...
CREATE TABLE IF NOT EXISTS copilot_sessions (
  ...
  COMMENT='v1.6.4.0 Copilot 私有会话（B组）';
...
CREATE TABLE IF NOT EXISTS copilot_previews (
  ...
  COMMENT='v1.6.4.0 Copilot 资料预览（B组，120秒有效）';
...
CREATE TABLE IF NOT EXISTS copilot_turns (
  ...
  COMMENT='v1.6.4.0 Copilot 异步轮次（B组）';
...
CREATE TABLE IF NOT EXISTS copilot_daily_budgets (
  ...
  COMMENT='v1.6.4.0 Copilot 日额度台账（B组）';
...
CREATE TABLE IF NOT EXISTS copilot_provider_attempts (
  ...
  COMMENT='v1.6.4.0 Copilot 模型调用台账（B组，不存响应正文/原始头）';
...
CREATE TABLE IF NOT EXISTS copilot_audit_events (
  ...
  COMMENT='v1.6.4.0 Copilot 审计元数据（B组，180天，不含正文）';
