...
1. **不修改任何产品代码、正式测试代码与部署脚本**，只新增本人取证脚本与证据目录；所有结论来自
   真实浏览器操作 + 数据库/接口反证。
2. O 止步前的第三轮证据（`docs/evidence/v1.6.3.5-uat3-o/`，12 个文件：过期心跳拒绝、刷新后完成、
   重扫新任务（65 表）、受理响应丢失恢复共四组截图与快照，附其夹具与全量回归 2126 用例）**已按
   Mr.Linsang 指示原样归档入库，内容未作任何改写**；本报告引用其为已确认事实，不重复执行，
   也不冒用其结论。
...
**1 项要求未落地（D-03）**，均给出照图施工级整改方案，建议随 v1.6.3.6 版本一并整改后复测。
