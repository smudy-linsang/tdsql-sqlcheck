...
# 第 2 轮复验结论（2026-07-07）：✅ 通过，准出

> 复验对象：commits `06d3f75`…`b358b2f`（缺陷修复 + 42条新单测 + 陈旧测试清理）
> 复验方式：与首轮同口径全实测（本地 MariaDB@13306 + uvicorn@8000）

| 复验口径（首轮报告第四章） | 结果 | 实测证据 |
|---|:-:|---|
| 1. 缺陷1/2 的 4 条自测 SQL | ✅ 4/4 | R117 不再误报 BIGINT UNSIGNED；R054 在 DDL 路径与元数据路径均正确命中"唯一索引不含分片键"，含分片键时不误报（解析器已补 UniqueColumnConstraint 处理 + raw_sql shardkey 回退） |
| 2. 新增单测文件且全绿 | ✅ | `tests/test_oracle_compat_rules.py` 实收 **103 个参数化用例**（结构5 + 命中42 + 通过42 + 防误报14），全部通过 |
| 3. `pytest tests/` 全量 0 failed | ✅ | **880 passed / 0 failed / 55 skipped**（首轮 34 failed 全部清零：缺陷1连带5个、口径断言10个、历史陈旧测试19个均已修复） |
| 4. 文档口径无残留 | ✅ | README/USER_GUIDE/功能使用手册均为119条；ARCHITECTURE.md:153 为 V1.0 版本历史记录按要求保留并已追加 V2.1 行 |
| 184 项验收脚本复跑 | ✅ 184/184 | 首轮唯一失败项 E2 已转绿；脚本被整改方做的改动经核对仅为 sys.path 与 GBK 控制台输出兼容，检测逻辑未削弱 |

**收尾说明**：验收中发现 `docs/功能使用手册.md` 第 74/844 行残留 V1.0 时代"22条"字样（首轮缺陷4描述中点到但未列入复验硬口径），属零风险文档字样，已由验收方随本报告顺手修正为 119 条，不再发起整改轮次。

**最终结论**：原厂《TDSQL兼容业务系统适配改造方案 V1.5.1》52 个规范点已按设计全部落地（新增 42 条规则 + 映射/增强既有 12 项 + 备案 5 项），规则在 5 个审核入口、规则集多租户与质量门禁全链路实测生效，全量回归零失败。**本项验收通过，允许准出。**
