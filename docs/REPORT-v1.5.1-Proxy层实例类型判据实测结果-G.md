...
| 项 | 内容 |
|---|---|
| **报告名称** | v1.5.1 Proxy 层实例类型判据实测结果报告 |
...
原先 `v1.5` 的问题在于 `probe_instance_type()` 简单地使用了 `if rows: votes_distributed = True`（只要有返回行就判分布式），而**忽略了返回行中的关键字段内容**。
...
> 🎯 **算法代码改造建议**：
> ```python
> has_cluster_tag = any(r.get("status_name") == "cluster" or "hash_range" in str(r.get("status_name")) for r in rows)
> if has_cluster_tag:
>     return "distributed"
> elif any(r.get("status_name") == "set" for r in rows):
>     return "centralized"
> ```
...
| **T01** | `/*proxy*/show status` | 返回 **2 行**（包含 `set_1782130875_4`，无 cluster，无 hash_range） | 返回 **8 行**（包含 `cluster`, `set_...1:hash_range`, `set_...3:hash_range`） | ⭐ **核心黄金判据** |
...
| **T06** | `show databases` | 返回 7 个库 (`tdsql_check2`) | 返回 8 个库 (`tdsql_check`, `xa`) | ℹ️ 弱（业务库差异） |
...
def probe_instance_type(self) -> tuple:
    """探测实例类型（V1.5.1 准确修正版）。"""
    detail = {"proxy_show_status": {}, "explain_check": {}}
...
本次实测圆满完成，拿到了**100% 具备说服力的第一手物理环境证据**。原样数据文件已存储于 `scratch/out_CENT.txt` 与 `scratch/out_DIST.txt` 供核验。

智能体 Q 可根据上述黄金判据完成 `v1.5.1` 的底层算法重构！
