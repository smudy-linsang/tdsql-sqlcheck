...
- 系统管理员在“平台治理 / 实例管理 / ZK发现配置”录入 ZK 地址、认证、根路径、驱动、Proxy 选择方式、默认业务库和内外网地址映射；配置保存后下一次扫描即时生效，无需重启服务。
- ZK 认证口令仅允许从浏览器写入，服务端以 Fernet 加密后存入系统元数据库；读取接口、页面、操作日志和发现响应均不回显口令或密文。口令输入框留空再保存表示保留旧值。数据库口令仅保留在服务端十分钟有效的发现会话中，导入后立即进入加密存储。
- 默认 Python 客户端在进程内完成 digest 认证；Shell 回退才以环境变量传给脚本并通过 `zkCli` 标准输入认证。

## 2. 配置方式与密钥前置

### 2.1 管理页面配置（推荐）

使用 admin 登录后进入“平台治理 / 实例管理”，点击 **ZK发现配置**。填写全部字段并保存：

1. **ZK 服务地址**：一个或多个 `主机:端口`，用逗号分隔；应用会逐节点尝试。
2. **ZK 根路径**：通常为 `/tdsqlzk`。
3. **发现驱动**：默认 `kazoo`；只有旧环境必须依赖 `zkCli.sh` 时才选择 `shell` 并填写该脚本绝对路径。
4. **认证用户名、认证口令**：首次保存必须填写口令；后续修改其他字段时口令可留空。
5. **内外网地址映射**：将 ZK 返回的内网 Proxy 主机映射到 CheckSQL 可连接的 NAT、VIP 或 FQDN；端口自动保持不变。

保存操作只允许 admin，扫描与导入仍按“实例管理”权限执行。元数据库加密密钥必须稳定可用：生产环境应从 KMS 或配置中心注入 `TDSQL_ENCRYPTION_KEY`，不能因重建容器或更换主机而丢失；否则历史配置无法正确解密。

### 2.2 旧部署环境变量回退

复制 [`../deploy/zk-discovery.env.example`](../deploy/zk-discovery.env.example) 中的非敏感配置至部署环境，
并由进程管理器、容器编排或受控环境文件注入。该方式只在系统元数据库尚未保存 ZK 配置时生效；管理员在页面保存后，数据库加密配置优先。不得把真实值提交到 Git、写入启动脚本或粘贴到任何仓库文件。
...
2. 默认发现驱动为 `kazoo`，运行镜像/主机只需具备 Python 依赖 `kazoo`，适用于 Windows、Linux 与 Docker；只有在配置页显式设为 `shell` 时才需 `bash`、`python3`、兼容的 `zkCli.sh` 和 Java。
