...
| 是否发生次生灾害 | **是** —— B-01 / B-02 / B-03 三项高优先级 |
...
### 【B-01｜高】大库历史报告静默丢失 96.7% 明细，而压缩本非必需
...
### 【B-02｜高】子表识别只用了命名匹配，违反本项目自己的既定规则 Rev.G / P1-03
...
### 【B-03｜高】BUG-02 的回归锁是假绿：把缺陷原样改回去，12 个用例仍然全过
...
### 【M-01｜中】去掉 `*2` 后余量归零，包限被调小时预检失守，且错误归因被误导
...
### 【M-02｜中】`skipped_list` 限长锁只加在最后一个写点，对峰值无效；注释声称的限制并不存在
...
### 【M-03｜中】完整性判据放宽后的 fail-open，且"跳过"在历史侧完全不可见
...
### 【N-01｜低】新代码里相邻两行，一行做了注释注入防护，一行没做
...
### 【N-02｜低】`compact_results_for_audit_history(results_json, job_id)` 的 `job_id` 形参从未使用
...
| 压缩把 `audit_columns_values` 写错列 | **未发生**。索引 8 与 21 列顺序核对一致（但无用例锁，见 B-03/M8） |
...
| 编号 | 优先级 | 事项 | 建议做法 |
|---|---|---|---|
| B-01 | 高 | 大库历史报告静默丢 96.7% 明细 | 阈值改为按 `max_allowed_packet` 自适应；若留压缩则必须加显式省略提示 + `omitted_count` 列 + 下载口优先读 ndjson |
| B-02 | 高 | 子表识别违反 Rev.G/P1-03 | 移植 `_classify_subpartitions` 三条件判据；`\d*` → `\d+` |
| B-03 | 高 | BUG-02 回归锁假绿 | 用例真调 `publish()`；补 M8/M9/M10 三条锁 |
| M-01 | 中 | 预检余量归零 + 错误归因被误导 | 系数取实测 1.25 或精确算转义长度；`rollback()` 加 try/except |
| M-02 | 中 | 限长锁加错位置 | 在 `extract_metadata` 返回处截断；删除与代码不符的 128 KiB 注释 |
| M-03 | 中 | fail-open + 跳过在历史侧不可见 | 跳过按原因分类；超阈值判 PARTIAL_SUCCESS；`audit_history` 增列；报告口径行补"跳过 N" |
| N-01 | 低 | `-- Object Name:` 未 sanitize | 加 `sanitize_comment` |
| N-02 | 低 | `job_id` 形参未使用 | 用起来或删掉 |
...
BUG-01、BUG-02 的主功能确实修好了，这点我实测确认。但 B-01 会让内网那个 6097 表库的
报告实际可用性严重下降（而且是在"修好了"的外表下丢数据），B-02 违反了本项目自己定过并
已有实现的规则、可能导致真业务表漏审，B-03 意味着 BUG-02 这个缺陷可以被原样改回去而
测试全绿 —— 这三条都够不上进 UAT 的门槛。

建议 Q 按第 6 节整改，我做第二轮 SIT。B-01 与 B-02 我会做定点复验 + 变异复验，
B-03 的三条新锁我会逐条做变异有效性验证。

---

*报告人：智能体D　送呈：Mr.Linsang*
