...
> v1.2 变更（评审整改）：修正 §5.3 SQL 骨架时间窗过滤列——由 `ts_min/ts_max` 改为**索引列 `timestramp`**（消除与 §0.1 的自相矛盾、避免生产监控库全索引扫描）；明确 `GROUP BY db,checksum` 即权威去重、Python 侧归一化仅用于展示；`rows_affected` 落库由"可选"升为 M1 强制。
...
WHERE db NOT IN ('mysql','information_schema','performance_schema','sys','tdsqlpcloud_monitor')
  AND db IS NOT NULL
  [AND timestramp >= :time_start]  -- ★时间窗过滤走 timestramp(采集时刻)，命中单列索引 index_time
  [AND timestramp <  :time_end]    --   与 §0.1、原厂 slow_sql_analysis.sh 口径一致；末端用 < 取次日00:00:00防漏
...
> ⚠️ **时间窗只能过滤 `timestramp`，不要用 `ts_min/ts_max`（评审整改，v1.2）**：`ts_min/ts_max` 虽有复合索引 `index_ts(ts_min,ts_max)`，但"时间窗重叠"式条件（`ts_max>=start AND ts_min<=end`）在复合索引上无法有效 seek、且 `ts_min<=end` 会命中几乎全部历史行，等同全索引扫描，在庞大的生产监控库上会拖慢 monitordb。`timestramp`（采集时刻）有单列索引 `index_time`，`timestramp>=start AND timestramp<end` 是紧致范围、毫秒级响应，且正是原厂口径。`ts_min/ts_max` **仅用于 SELECT 里展示首/末执行时间**（`MIN(ts_min)/MAX(ts_max)`），不进 WHERE。
>
> **`checksum` 即权威去重键（评审整改，v1.2）**：monitordb 的 `checksum` 由监控采集器为每类 SQL 算好，`GROUP BY db, checksum` 在 SQL 层即完成精确聚合，**Python 侧无需再做重活的"二次聚合分组"**。Python 侧的指纹归一化（去空格/去分号/括号归一）**只用于展示美化** `fingerprint` 文本，降低 CPU 开销。（可选精修：若发现极少数逻辑相同但 `checksum` 不同的跨 SET 变体，可在 Python 侧对已聚合结果做一次"按归一化 fingerprint 合并"的轻量补充，不是必须。）
...
- 落库映射沿用现有循环（因键名已对齐）。**M1 必做增量（评审整改，v1.2 由"可选"升为"强制"）**：给 slow_queries 加列 `rows_affected BIGINT DEFAULT 0` 并在 INSERT 补该字段（§5.3 已带出 `SUM(rows_affected_sum)`/`MAX(rows_affected_max)`）。`rows_affected` 能直接反映 UPDATE/DELETE 实际影响了多少数据，是 monitordb 独有的高价值维度，前端慢SQL详情页应展示；数据源回退到 `digest` 时该列存默认 `0`。
