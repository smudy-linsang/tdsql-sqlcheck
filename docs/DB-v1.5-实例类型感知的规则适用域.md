...
| P2 | **NULL 即"未知口径"** | 存量记录的 `instance_type` 一律为 `NULL`，语义是"v1.5 之前产生，评估口径未知"。**严禁回填** —— 回填等于伪造历史评估口径，会让扫描对比得出错误结论 |
...
| `audit_history` | 新增列 | 3 | 报告口径留痕，支撑报告横幅与跨口径校验 |
| `scan_snapshots` | 新增列 | 1 | 快照口径留痕，支撑扫描对比的口径隔离 |
...
| `skipped_rules_count` | INT | ❌ | 报告横幅数据源："本次按【集中式】口径评估，已跳过 25 条仅分布式适用规则"。**冗余存储是有意的**——规则清单会随版本变化，历史报告必须能显示当时的真实数字，不能事后重算 |
...
- 报告页只需要一个数字做横幅；真要看是哪 25 条，`GET /api/v1/rules?instance_type=centralized` 随时可查当前清单；
...
**问题（关键）**：快照两两相减得出"整改率"。v1.5 上线后，同一台**集中式**实例的问题数会因为不再跑 25 条分布式规则而显著下降——如果拿 v1.5 前的快照做基线，对比报告会呈现一次**根本没有发生过的整改**。

这和 v1.4 花大力气消灭的"换个低尺度项目再扫一次，问题就变少了"是**同一类伪命题**，必须在同一个地方堵住。
...
ALTER TABLE scan_snapshots
    ADD COLUMN instance_type VARCHAR(16) NULL DEFAULT NULL
        COMMENT '采集本快照时的实例类型口径；NULL=v1.5前快照，口径未知，跨版本对比需警示';
```

#### 对比时的口径校验（三态判定）

| 基线 `instance_type` | 对比 `instance_type` | 行为 |
|---|---|---|
| 同值（如都 `centralized`） | 同值 | ✅ 正常对比 |
| `distributed` | `centralized`（或反之） | ❌ **拒绝生成对比报告**，返回 400 + 明确原因 |
| NULL（v1.5 前快照） | 任意 | ⚠️ **允许对比，但报告顶部打醒目警示**："基线快照采集于 v1.5 之前，评估口径未知。问题数变化可能包含口径调整因素，不代表真实整改成效。" |

**为什么 NULL 是"警示"而不是"拒绝"**：

存量快照是用户真金白银积累的基线资产。一刀切拒绝，等于让所有历史基线在升级当天集体作废，用户会失去纵向趋势。而"允许 + 醒目警示"把判断权交还给人：升级前后跨越的那一次对比要打折看，之后的对比恢复正常。

**为什么类型不同是"拒绝"而不是"警示"**：

不同类型的实例本来就是不同的物理对象，拿分布式实例的快照当集中式实例的基线，即使没有 v1.5 的口径问题也毫无意义。这是**用户操作错误**，应当明确拦下，而不是给个警示让他继续。

#### 索引

**不加。** `scan_snapshots` 的查询入口是 `module + connection_id + scan_finished_at`，`instance_type` 只在取到具体两行之后做值比较，不参与筛选。
...
1. **安全方向正确**。兜底走 `distributed` = 跑全部 119 条规则 = **宁可多报不可漏报**。反过来兜底走 `centralized`，会让所有无实例上下文的审核静默少跑 25 条规则——而漏报是看不见的（报告里少的东西没人会发现），这是最危险的失效模式。
...
-- ── D-3 基线快照：支撑对比的口径隔离 ──
-- 缺此列则 V1.5 上线后集中式实例会凭空产生一次虚假"整改成效"，
-- 与 V1.4 消灭的"换项目刷低问题数"属同一类伪命题。
ALTER TABLE scan_snapshots
    ADD COLUMN instance_type VARCHAR(16) NULL DEFAULT NULL
        COMMENT '采集时的实例类型口径；NULL=V1.5前快照，跨版本对比需警示';
...
-- 任何 UPDATE 回填都是在伪造这些报告当时的评估口径，会让扫描对比得出错误结论。
```
