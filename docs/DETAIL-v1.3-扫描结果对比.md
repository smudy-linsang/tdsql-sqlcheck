...
```jsonc
{
  ...
  "meta": {
    ...
    "time_window_end": "",
    "node": "set_1782132369_1",        // M 补充: TDSQL 分布式节点(set)区分位
    "created_by": "admin"
  },
  "stats": {
...
> **M 补充 6 [截断后行为]**: 截断时按 ERROR > WARNING > INFO 优先级保留，**截断发生在写库前**（不破坏 base/target 的"同问题项"判定）。报告顶部以红色 banner 提示"快照已被截断，本报告仅展示前 20000 条问题项（按级别降序），完整数据请在页面查看/导出"，并附 `truncated_count` 字段说明被截掉的条数。
...
> **M 补充 2 [算法版本化迁移]**: 指纹算法升级时（v1→v2）需同步：
> 1. 修改 `FINGERPRINT_ALGO = "v2"`
> 2. 提供迁移脚本将所有 `fingerprint_algo='v1'` 的快照重算并更新字段，或在比对引擎中同时支持新旧两套指纹的混合比对
> 3. 不允许直接废弃旧指纹（会导致历史快照全部不可比）
...
def extract(results, db_name: str, node: str = "") -> tuple[list[IssueItem], int]:
    ...
    for r in results:
        ...
        for v in (r.violations or []):
            ...
            seq = per_obj_rule[ck]
            key = _fp("schema_audit", node, obj_name, obj_type, v.rule_id, disc,
                      "" if seq == 1 else str(seq))   # M 补充 1: 指纹含 node
...
├ ⚠️ 提示区（可比性告警：时间窗口不一致 / 快照被截断 / 跨 set 提示）
