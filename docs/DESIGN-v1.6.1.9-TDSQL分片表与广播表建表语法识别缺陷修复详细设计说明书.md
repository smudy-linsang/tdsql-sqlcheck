...
| 撰写 | 智能体 A |
| 修订 | **Rev.D**——按智能体 O 独立评审（BLOCK）整改，7 项强制项全部落实（见 §13 逐条答复） |
| 状态 | **待复审** |
...
| `TDSQL_DISTRIBUTED BY HASH(col)` | **本项目生产内核已验证输出** | 本次生产 `SHOW CREATE TABLE`（#3）+ TDSQL 管理平台截图显示该表类型为「hash分片表」+ 用户确认。公开基础建表文档当前主要写作 `shardkey=col`，`TDSQL_DISTRIBUTED BY HASH` 见于 DTS 二级分区材料，存在版本口径差异 |

> **📌 施工前置交付物**：需补录该内网环境的 **TDSQL 内核 / Proxy 版本号**，写入本节。若后续取得该版本官方手册，把链接补入。**不得声称所有 TDSQL 版本均输出相同 HASH 形态。**
...
| **NJ-3** | J-3 是**每一个**唯一索引都要满足，不是"任意一个" | 因此唯一索引**不得展平成一个列并集**，必须逐个判断（O 评审 §6.4） |
...
| **ADJ-6** | `R077` 的 `BROADCAST` 快速通道 | `BROADCAST` 与真实 `shardkey=col` 冲突声明时不做冲突检测 | 见 §8.3 | Phase 2 可选；本次以特征化测试锁定现状 |
...
**处置**：本次**不改**（NG-5），但把上表两行落为**特征化测试**锁定现状——将来行为一旦变化，测试立即报警。列为 ADJ-6 交用户决策是否 Phase 2 处理。
...
| **Rev.D**（本版） | **智能体 O 独立评审（BLOCK）** | **见 §13 逐条答复**：7 项强制整改全部落实；撤回 Rev.C 的错误论证；新增清洗+尾部锚定、精确哨兵、R054 共享 HASH、J-3 逐索引判定、文案同步、X1–X13 反例 |
...
- [ ] 写明 ADJ-1/2/3 留待 Phase 2（ADJ-3 优先）；ADJ-4 已关闭；ADJ-5 为原子变更约束；ADJ-6 待决策
- [ ] 补录内网 TDSQL 内核/Proxy 版本号到 §1.1
