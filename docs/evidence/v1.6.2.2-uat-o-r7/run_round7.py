"""Independent seventh-round UAT runners; never overwrite prior evidence."""
import argparse
import json
import os
...
def prepare(env):
    return subprocess.run(
        [sys.executable, str(R1 / "prepare_regression.py")],
...
def full():
    env = base_env("tdsql_uat_o_reg_r7_full_20260829")
    if prepare(env):
        return 2
    return run(
...
def probes():
    env = base_env("tdsql_uat_o_reg_r7_probe_20260829")
    jobs = [
    ...
    for name in ("rule_probe_current.json", "edge_current.json",
        ...
        old = json.loads((R6 / name).read_text(encoding="utf-8"))
        new = json.loads((HERE / name).read_text(encoding="utf-8"))
        comparisons[name] = {"equal": old == new}
    (HERE / "round7_diff.json").write_text(
        json.dumps({
            "baseline_commit": "8fee172a67daacc42809e173ecd20c5e3b8aac1b",
...
def matrix():
    env = base_env("tdsql_uat_o_reg_r7_matrix_20260829")
    if prepare(env):
        return 4
    results = []
