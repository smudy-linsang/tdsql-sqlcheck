...
"""
import ast, io, sys, difflib, collections, contextlib
...
def update_design(design_path):
    ...
    text = io.open(design_path, encoding="utf-8").read()
    begin = "<!-- BEGIN AUTOGENERATED CODESTAT: docs/evidence/v1.6.2.2/codestat.py -->"
    end = "<!-- END AUTOGENERATED CODESTAT -->"
    if text.count(begin) != 1 or text.count(end) != 1:
        raise AssertionError("codestat markers must occur exactly once")
    left, rest = text.split(begin, 1)
    _old, right = rest.split(end, 1)
    io.open(design_path, "w", encoding="utf-8", newline="\n").write(
        left + begin + "\n" + generated + "\n" + end + right)
    return 0


if __name__ == "__main__":
    if len(sys.argv) == 5 and sys.argv[3] == "--update-design":
        sys.exit(update_design(sys.argv[4]))
    sys.exit(main())
