...
"""
import sys, os, collections, io, contextlib
...
def update_design(path):
    ...
    begin = "<!-- BEGIN AUTOGENERATED MANIFEST TABLES: docs/evidence/v1.6.2.2/manifest_doc.py -->"
    end = "<!-- END AUTOGENERATED MANIFEST TABLES -->"
    if text.count(begin) != 1 or text.count(end) != 1:
        raise AssertionError("manifest markers must occur exactly once")
...
if __name__ == "__main__":
    if len(sys.argv) == 3 and sys.argv[1] == "--update-design":
        update_design(sys.argv[2])
    else:
        main()
