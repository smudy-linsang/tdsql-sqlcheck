"""Prepare isolated local fixtures for v1.6.3.0 G14 UAT round 1.
...
META_DB = "tdsql_uat_o_g14_r1_1630"
TARGET_A = "tdsql_uat_g14_r1_a"
TARGET_B = "tdsql_uat_g14_r1_b"
TARGET_EMPTY = "tdsql_uat_g14_r1_empty"
...
def main():
    ...
    for role in ("dba", "developer", "auditor"):
        username = f"uat_g14_{role}"
        if not auth_service.get_user(username, use_cache=False):
            _, error = auth_service.create_user(
                username, password, role, f"G14 UAT {role}", "G14 UAT fixture")
            if error:
                raise RuntimeError(error)

    role_id = "uat_g14_tabletype_only"
    result = create_custom_role(
        role_id, "G14 表类型统计最小权限", "仅可进入深度诊断表类型统计")
    ...
    if not auth_service.get_user("uat_g14_tt", use_cache=False):
        _, error = auth_service.create_user(
            "uat_g14_tt", password, role_id,
            "G14 UAT 最小权限", "G14 UAT fixture")
        if error:
            raise RuntimeError(error)
    ...
    try:
        conn.execute(
            ...
            "WHERE username IN ('admin','uat_g14_dba','uat_g14_developer',"
            "'uat_g14_auditor','uat_g14_tt')")
        conn.commit()
    finally:
        conn.close()
    ...
    shared = dict(
        ...
        charset="utf8mb4", operator="UAT-O",
    )
    registry.save_connection(
        name="G14 UAT 集中式样本", database=TARGET_A, is_default=True,
        is_distributed=False, conn_id="uat_g14_central",
        description="Synthetic MariaDB centralized UAT fixture", **shared)
    registry.save_connection(
        name="G14 UAT 分布式失败样本", database=TARGET_B, is_default=False,
        is_distributed=True, conn_id="uat_g14_distributed",
        description="Synthetic MariaDB forced-distributed error-path fixture", **shared)
    registry.save_connection(
        name="G14 UAT 离线样本", host="127.0.0.1", port=1,
        username="synthetic", password="", database="offline", is_default=False,
        is_distributed=True, conn_id="uat_g14_offline", operator="UAT-O",
        description="Intentional unavailable endpoint")

    print(
        "G14_UAT_FIXTURES_READY "
