"""Isolated round-three runners; preserve the signed round-one evidence."""
import argparse
import io
import json
import os
...
def main():
    ...
    if a.mode in ('full', 'matrix'):
        env['SQLCHECK_DB_NAME'] = 'tdsql_uat_o_reg_r3_' + a.mode + '_20260828'
    ...
    temp = Path(tempfile.mkdtemp(prefix='uat-o-r3-before-'))
    with zipfile.ZipFile(io.BytesIO(subprocess.check_output(['git','archive','--format=zip','d9fad2a'], cwd=ROOT))) as z:
        z.extractall(temp)
    code = run([sys.executable, str(HERE.parent / 'v1.6.2.2-uat-o-r2' / 'edge_probe.py'), '--repo', str(temp), '--out', str(HERE / 'edge_baseline.json')], 'edge_baseline.txt', env)
    if code:
        raise SystemExit(code)
    before = json.loads((HERE.parent / 'v1.6.2.2-uat-o-r2' / 'rule_probe_current.json').read_text(encoding='utf-8'))
    ...
    obj = {'before_commit':'d9fad2a', 'tested_commit':subprocess.check_output(['git','rev-parse','HEAD'], cwd=ROOT, text=True).strip(), 'baseline_temp':str(temp), 'rules_equal':before['rules']==after['rules'], 'case_count':1000, 'changes':changes, 'remaining_failures':after['failures'], 'edge_changes':edge_changes}
    (HERE / 'round3_diff.json').write_text(json.dumps(obj, ensure_ascii=False, indent=2), encoding='utf-8', newline='\n')
