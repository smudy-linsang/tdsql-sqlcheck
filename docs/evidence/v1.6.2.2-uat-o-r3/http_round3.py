...
def main():
    ...
    for row in controls:
        for entry in ('file','upload'):
            if entry == 'file':
                r = s.post(BASE + '/api/v1/audit/file',json={'content':row['sql'],'file_path':'uat_r3_guard.sql','instance_type':'distributed'},timeout=30)
            else:
                r = s.post(BASE + '/api/v1/audit/upload',files={'file':('uat_r3_guard.sql',row['sql'],'text/plain')},data={'instance_type':'distributed'},timeout=30)
    ...
    for user in ('uat_o_developer','uat_o_auditor','uat_o_dba'):
        ...
        if user != 'uat_o_dba':
            r = role_s.post(BASE+'/api/v1/tdsql/connections',json={'name':'uat_r3_forbidden','host':'127.0.0.1','port':1,'username':'synthetic','password':''},timeout=20)
