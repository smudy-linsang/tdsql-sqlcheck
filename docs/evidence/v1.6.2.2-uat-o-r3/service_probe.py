...
import sys
if not os.environ.get('SQLCHECK_DB_NAME','').startswith('tdsql_uat_o_reg_r3_'):
    raise SystemExit('Isolated round-three diagnostic database required')
...
for rec in records:
    ...
    result, gate, context = service.audit_single_sql(case['sql'],created_by='UAT-O-R3',instance_type=scope)
