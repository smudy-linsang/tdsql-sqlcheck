"""Prepare isolated sixth-round browser fixtures; synthetic local data only."""
import os
...
if os.environ.get("SQLCHECK_DB_NAME") != "tdsql_uat_o_r6_1622_20260829":
    raise SystemExit("Refusing non-round-six database")
...
users = (
    ("uat_o_admin_r6", "admin", "UAT-O 管理员"),
    ("uat_o_dba_r6", "dba", "UAT-O DBA"),
    ("uat_o_developer_r6", "developer", "UAT-O 开发"),
    ("uat_o_auditor_r6", "auditor", "UAT-O 审计"),
)
for username, role, display in users:
    if not auth_service.get_user(username, use_cache=False):
        _, error = auth_service.create_user(
            username, password, role, display, "UAT-O R6 synthetic fixture")
        if error:
...
target = "tdsql_uat_o_r6_index_target"
explain_target = "tdsql_uat_o_r6_explain_target"
...
try:
    with raw.cursor() as cursor:
        ...
        cursor.execute(
            ...
            "ENGINE=InnoDB COMMENT='UAT O R6 synthetic duplicate index fixture'"
        )
        cursor.execute(
...
for conn_id, name, port, database in (
    ("uat_o_index_r6", "UAT-O-R6 索引重复样本", MYSQL_CONFIG["port"], target),
    ("uat_o_local_r6", "UAT-O-R6 本地在线样本", MYSQL_CONFIG["port"], explain_target),
    ("uat_o_offline_r6", "UAT-O-R6 离线样本", 65530, target),
):
...
conn = _get_connection()
row = conn.execute(
    "SELECT id FROM slow_queries WHERE db_name=?", ("uat_o_r6_workflow",)
).fetchone()
conn.close()
if not row:
    row = SlowQueryService().add_slow_query(SlowQueryRecord(
        ...
        db_name="uat_o_r6_workflow", exec_count=10, avg_time_ms=1200,
...
(HERE / "gateway_xss.log").write_text(
    ...
    "sql=select 1 /* </script><script>window.__uat_o_r6_pwned=1</script> "
    "<img src=x onerror=window.__uat_o_r6_img=1> */&db=synthetic&user=uat\n",
    encoding="utf-8", newline="\n",
)
