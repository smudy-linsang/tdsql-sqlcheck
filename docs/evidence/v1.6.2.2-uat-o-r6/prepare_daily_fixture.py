"""Create successful comparison rows for sixth-round stale-result browser testing."""
import os
...
try:
    connection_id = "uat_o_index_r6"
    dates = ("2026-08-26", "2026-08-27")
    conn.execute(
        "DELETE FROM daily_inspection WHERE connection_id=? AND inspect_date IN (?,?)",
        (connection_id, *dates),
    )
    columns = [
        row["Field"] for row in conn.execute("SHOW COLUMNS FROM daily_inspection").fetchall()
    ...
    for date_value, cpu_value in zip(dates, (21.0, 42.0)):
        ...
        for name in columns:
            values.append({
                ...
                "node": "uat-node-r6", "cpu_peak": cpu_value,
