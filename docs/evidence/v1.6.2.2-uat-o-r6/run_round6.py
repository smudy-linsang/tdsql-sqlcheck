"""Independent sixth-round UAT runners; never overwrite prior evidence."""
import argparse
import json
import os
...
def run(args, log, env):
    with (HERE / log).open("w", encoding="utf-8", newline="\n") as out:
        result = subprocess.run(
            args, cwd=ROOT, env=env, stdout=out, stderr=subprocess.STDOUT,
...
def start_test_server(env):
    ...
    for _ in range(80):
        if process.poll() is not None:
            raise RuntimeError(f"test server exited early: {process.returncode}")
        try:
...
def full():
    env = base_env("tdsql_uat_o_reg_r6_full_20260829")
...
def probes():
    env = base_env("tdsql_uat_o_reg_r6_probe_20260829")
    jobs = [
    ...
    for name in ("rule_probe_current.json", "edge_current.json",
        ...
        old = json.loads((R5 / name).read_text(encoding="utf-8"))
        new = json.loads((HERE / name).read_text(encoding="utf-8"))
        comparisons[name] = {"equal": old == new}
    (HERE / "round6_diff.json").write_text(
        json.dumps({
            "baseline_commit": "50a1c049b49407c8055ec888735dcd578fe230b2",
...
def matrix():
    env = base_env("tdsql_uat_o_reg_r6_matrix_20260829")
