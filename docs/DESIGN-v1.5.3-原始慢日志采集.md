...
### 16.2 开发环境真实日志格式证据（v1.6.0.0）

2026-08-02 已在获授权的开发 Proxy 日志主机完成只读验证：实际日志 glob 为
`/data/tdsql_run/<proxy-port>/gateway/log/slow_sql_instance_<proxy-port>.*`，并确认
`# Time` 使用 `YYMMDD HH:MM:SS 微秒` 的空格分隔紧凑格式。对应解析支持和回归证据见
[`TEST-v1.6.0.0-原始慢日志真实环境端到端测试报告_智能体O.md`](TEST-v1.6.0.0-原始慢日志真实环境端到端测试报告_智能体O.md)。

该证据只证明日志格式及程序可用性；以下仍为**启用前强制门禁，不是可选建议**：
