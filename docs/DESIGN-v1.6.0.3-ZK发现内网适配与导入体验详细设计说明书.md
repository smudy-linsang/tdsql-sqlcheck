...
- 以业务账号对**适配后**的每个 Proxy 执行 `SHOW DATABASES`（精确排除系统库与 MonitorDB 库）。
  > **口径更正（v1.6.0.5/v1.6.0.6）**：原沿用 v1.6.0.1 的"任一失败即 `BUSINESS_PROXY_INCOMPLETE` 从严口径"已作废。
  > 内网实测证明分布式与集中式普遍双 Proxy（主+备），备不可达会让枚举大面积失败。
  > 现行口径：逐端点捕获异常，**≥1 成功即可**；0 成功→`NO_AVAILABLE_PROXY`；
  > 有失败或目录不一致→取并集并标 `proxy_show_partial`（R-15：宁可多给可见的错误，不可少给不可见的错误）。
  > 扫描富集与导入预检两条枚举路径口径一致（v1.6.0.6 A-P1-01 对齐）；
