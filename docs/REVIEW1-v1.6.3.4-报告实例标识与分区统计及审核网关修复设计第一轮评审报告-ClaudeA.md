...
| P2 | P2-02 | REQ-01 对**文件审核报告**（H01，平台最常用的报告）给出"未关联实例（离线文件审核）"，与需求 ①"所有出具的 html 报告中均要添加扫描或检查的实例连接名称"的字面要求存在张力 | 这是需求解释分歧，不是技术缺陷。但设计单方面选定了一种读法而没有把它标成待裁决项；实测 `audit_file` API **本来就接收 `connection_id`**，只是前端没绑 |
| P2 | P2-03 | `GATEWAY_MAX_CONCURRENT=1` 是**新增的用户可见限制**（当前无任何并发限制），默认不排队直接 429，而单次分析上限 540 秒 | 多 DBA 团队里第二个人最长被挡 9 分钟。设计把它放在配置表里，没有标成需要业务方接受的行为变更 |
...
### P2-02（需 Mr.Linsang 决策）REQ-01 在"最常用的那个报告"上给出的是"未关联实例"，与需求原文存在口径差；且现有 API 参数带有门禁副作用
...
### P2-03（需 Mr.Linsang 决策）`GATEWAY_MAX_CONCURRENT=1` 且默认无排队，是一项新增的用户可见限制
...
|---|---|---|---|
|①|所有出具的 HTML 报告均添加扫描/检查的**实例连接名称**|§3 REQ-01，H01—H14 共 14 个出口 + `report_context.py` + `v14/140`|**基本覆盖，1 项待裁定**。我逐条核对了 H01—H14 与仓库实际 HTML 出口，**未发现遗漏的出口**；H01 的"未关联"口径需 Mr.Linsang 拍板（P2-02）|
...
|④|内网大网关日志分析上传失败，**分析失败原因并修复**|§6 REQ-04，专属中间件 + 流式落盘 + 受控子进程 + `v14/142`|**覆盖充分**。设计不仅指出内网诊断报告漏掉的应用层 50 MiB 限额，还纠正了该报告的三处事实错误；缺口是并发策略需知情接受（P2-03）与超时链需收口（P3-01）|
...
|P2-02|P2|H01 文件审核报告"未关联实例"的口径差；且 `connection_id` 带门禁副作用|**Mr.Linsang 裁定** → O 落文|三选项写入设计并标注裁定结果；若选 B，须补"传 `report_connection_id` 不产生 gate_result"回归锁|
|P2-03|P2|`GATEWAY_MAX_CONCURRENT=1` 无排队是新增用户可见限制|**Mr.Linsang 裁定** → O 落文|§6.3 新增知情项段落并标注裁定结果；§8 补并发 429 用例|
...
3. P2-02、P2-03 已取得 Mr.Linsang 的明确裁定并写入文档（裁定内容本身我不评价，只核验"已裁定且已落文"）。

P3 三条不构成阻断，但建议与上述一并改完，避免二轮再开一次。

## 7. 需要 Mr.Linsang 拍板的两件事

这两件都不是技术对错问题，是产品口径问题，我不能替您决定：

**其一，文件审核报告要不要显示实例名称。** 文件审核是拿一段 SQL 文本来审，它和任何一个数据库实例都没有真实关联。设计给的是"未关联实例（离线文件审核）"，技术上诚实；但您的原话是"所有报告都要有实例连接名称"，这张恰恰是您用得最多的报告之一。我建议的折中是 **B 方案**：页面上给一个**可选的**"关联实例（仅用于报告标识）"下拉，选了就显示真名，不选就显示"未关联"。要特别提醒的是——现有后端接口里 `connection_id` 一旦传了，会**顺带打开门禁评估**，把文件审核报告变成带"通过/不通过"结论的报告。所以 B 方案必须走一个**新的独立字段**，绝不能图省事复用 `connection_id`，否则就是典型的"改一处伤一片"。

**其二，网关日志分析要不要限制成同一时刻只能一个人跑。** 现在是没限制的（所以才会有内网那次失败）。设计要改成"同时只允许一个"，第二个人直接被拒，最长可能持续约 10 分钟。这个限制本身是必要的——一次分析要吃掉 2 GiB 临时空间和大量 CPU，两个人同时跑更容易一起失败。我建议选 **(b)**：仍然拒绝，但告诉对方"当前有任务在跑，约 N 分钟后可重试"，并且明确写"您的文件没有被处理，请稍后重传"，别让人以为已经排上队了。

## 8. 评审边界声明

1. 本轮**未修改任何代码**，符合 Mr.Linsang "切记你不要动代码"的要求。本文档是本轮唯一交付物。
