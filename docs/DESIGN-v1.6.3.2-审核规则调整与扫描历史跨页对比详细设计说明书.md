...
8. 规则总数由 119 条变为 121 条；分布式实例生效 121 条，集中式实例生效 90 条，集中式因适用域跳过 31 条（Rev.E/GATE-2：R031 同步改仅分布式后由原 91/30 调整为 90/31）。
...
> **Rev.E / GATE-2（林桑签署决议 §二，2026-09-05）**：上述 OUT-01 边界（R031 保持 ALL、集中式仍拦截函数、“自定义函数不属于零覆盖”）已被 DBA 正式“另行评审 R031”授权推翻——R031 同步改为 `instance_scope = DISTRIBUTED`。集中式对自定义函数也变为零覆盖，与视图/存储过程/触发器/临时表一致，消除“放行视图/过程/触发器却拦截函数”的割裂。集中式实际生效 91→**90**、按适用域跳过 30→**31**、仅分布式规则 30→**31**。权威依据为《GATE-DECISION-v1.6.3.2-生产发布门禁签署决议与整改任务书》。
...
> **Rev.E / GATE-3（林桑签署决议 §三，2026-09-05）**：上表「CREATE 二级 bare `LESS THAN MAXVALUE` → 至少含 E999」的失败关闭口径已被 DBA **拒签推翻**。bare MAXVALUE 是 MySQL/TDSQL 官方合法写法，用 E999 兜底会因 `ast=None` 引发 R003/R004/R005/R028/R118 级联假阳性（DBA 定性“用假阳性语法错误兜底业务拦截不可接受”）。**定版改为**：在 `parse_one` 之前把 bare 形态归一化为语义等价的 `VALUES LESS THAN (MAXVALUE)`（仅改字符串/注释外代码段、`raw_sql` 保持原文），sqlglot 正常解析为 Create，主键/引擎/字符集/列约束正常提取、级联假阳性归零；R121 仍由独立 token 策略扫描命中。据此上表 CREATE bare 行更新为「distributed 含 R121、**不含 E999**；centralized 不含 R121/E999」，与括号形态一致；DEF-SIT-02 引入的合成守卫已删除。权威依据为《GATE-DECISION-v1.6.3.2》。
...
| 其他分类 | 不变 | 不变 |
| 仅分布式规则 | 27 | 31（Rev.E/GATE-2） |
| 分布式实例实际生效 | 119 | 121 |
| 集中式实例实际生效 | 92 | 90（Rev.E/GATE-2） |
| 集中式按适用域跳过 | 27 | 31（Rev.E/GATE-2） |

计算说明：R120 是通用规则，新增后集中式先增加 1；R030、R032 改为仅分布式又减少 2，因此集中式从 92 变为 91。**Rev.E / GATE-2**：R031 经 DBA 授权同步改为仅分布式，集中式再减 1 → **90**，仅分布式规则与跳过数各 +1 → **31**。R121 仅分布式，不增加集中式数量。
...
| Rev.E | 2026-09-05 | 按林桑《GATE-DECISION-v1.6.3.2 生产发布门禁签署决议与整改任务书》同步两项授权变更：**GATE-2** 推翻 §4.3 OUT-01，R031 改仅分布式（§6.2 集中式生效 90 / 跳过 31 / 仅分布式 31）；**GATE-3** 推翻 §4.7.5 bare MAXVALUE 失败关闭口径，改为 parse_one 前归一化消除级联假阳性、删除 DEF-SIT-02 合成守卫。权威依据为 GATE-DECISION 文档 |
