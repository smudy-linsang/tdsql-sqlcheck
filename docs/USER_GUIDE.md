...
<!-- v1.6.4.0 / CP-1：AI Copilot 专家助手使用说明 -->

## 0. AI Copilot 专家助手（v1.6.4.0 新增）

Copilot 专家助手是平台内置的只读建议助手，帮助理解审核结果、排查任务失败、解释规则与报告。

### 0.1 入口

- **页头 Copilot 图标**：全局抽屉，快速提问；
- **侧栏「Copilot专家助手」**：会话列表 + 完整对话页；
- **业务结果区**：选中记录后点「解读/解释」类按钮（仅在有对应模块权限时出现）。
...
|---|---|
| 解释违规规则与适用域 | 代替正式审核结论 |
| 整理已有记录的事实摘要 | 在数据库执行 SQL/DDL |
| 给出候选修改建议（纯文本复核） | 声称语义等价或性能已改善 |
| 排查任务失败的已知错误码 | 自动建索引/改参数/触发扫描 |
| 本地使用指南问答（无模型也可用） | 读取业务行数据/原始日志/凭据 |

### 0.3 使用流程

1. 选择场景（使用帮助/解释规则/SQL 修改建议/解读审核结果/任务失败排查等）；
2. 输入问题，必要时选择要带入的记录；
3. 点「预览资料」——确认本次将发送的脱敏资料与目标域；
4. 确认后提交，助手后台处理（约数秒），完成后查看回答；
5. 回答区分「AI 建议 / 本地帮助 / 降级回答」，并附来源与限制说明。

### 0.4 隐私与安全

- 每次提问前可预览实际发送的资料；
- 口令、密钥、原始日志、业务行数据不会被发送；
- 会话仅本人可见；管理员也不默认阅读他人会话正文；
- 模型未配置时助手退为本地帮助，不影响原审核/采集/导出。
...
| SQL审核 | 基于121条规则审核SQL语句（支持项目级规则集差异化） |
