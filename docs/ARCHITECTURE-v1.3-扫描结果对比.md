...
| 文档状态 | 待评审（v2，按 1.2.0.9 最新代码重核重写） |
| 编制 | 智能体 A |

> **版本说明**：本文档 v1 依据 V1.2.0.7 编写。V1.2.0.8/1.2.0.9 期间的变更
> （日常巡检对比分页与单机聚焦、概览趋势图渲染修复、发布打包脚本）**均未触及本设计涉及的
> 三个目标模块**；文中引用的全部代码事实已在 V1.2.0.9 上逐项复核并更新标注。
...
| 模块 | 指纹构成 |
|---|---|
| 在线元数据审核 | `schema.对象名` + `对象类型` + `rule_id` (+ 归一化消息区分位，用于同对象同规则多次命中) |
...
| R5 快照 JSON 过大拖慢比对 | 性能劣化 | 快照仅存比对必需字段；单份超阈值（20000 项）截断并标记 `truncated=true`，报告提示 |
| R6 与其他开发并行冲突 | 合并冲突、返工 | 本设计以新增文件为主，改造点集中且明确（《详细设计》§2.2 清单）；开发前对齐改造文件清单，避免同段并行修改 |
...
**关联文档**
- 《扫描结果纵向对比能力 详细设计说明书》 — 照图施工级实现细节（含 **D1/D2/D4 缺陷修复施工方案 §11**）
