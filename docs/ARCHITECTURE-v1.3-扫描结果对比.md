...
> **M 合并说明 → 领导决策 2026-07-26**: G 的"最近 10 次/30 天"与 A 的"365 天"差异**已消解**。领导决策为**统一保留 365 天**（`scan_snapshots` 和 `scan_compare_reports` 一致），不做差异化。G 提出的"应用层快速窗口"作为前端列表的默认筛选范围（**仅展示**最近 30 天内的快照以保证 UI 性能），不影响后端存储周期；管理员可通过 `POST /api/v1/admin/retention/override` 调整个例外。
...
- **M 新增 7 项关键补充**：
  ...
  7. 留档与快照保留策略统一（均 365 天 — **领导决策 2026-07-26 固化**）
