...
| `E4006` | 400 | 模块类型缺失或不支持（`module` 未传 / 取值非枚举） |
| `E4031` | 403 | 无该模块数据的访问权限 |

> **必填参数缺失的处理（v1.3.0.1）**：`module` 在接口签名上声明为可选、由端点内手动校验，
> 缺失时返回 **400 `E4006`** 而非 FastAPI 默认的 422。原因：422 的响应体不含 `code` 字段，
> 会破坏 §1.3 "失败一律返回 `{detail, code}`" 的约定；且平台既有先例
> （`daily_inspect.compare` 对 `connection_id`）同样返回 400 + 明确提示。
