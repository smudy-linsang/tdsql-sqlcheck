# v1.6.3.7 终态收敛缺口与前端「活动任务」判定整改开发记录

| 项 | 内容 |
|---|---|
| 版本 | v1.6.3.7（由 v1.6.3.6 递进） |
...
- **全量受控回归**（同一 docker MySQL 8.0 / `max_allowed_packet=64MiB` 环境）：
  ...
  | head（本次 v1.6.3.7） | 419 | **1606 (+10)** | 114 | 31 |
...
`VERSION`、`config.APP_VERSION`、`index.html` 5 处（title / 2×css `?v=` / version div / js `?v=`）
由 `1.6.3.6` → `1.6.3.7`；`index.html:396` 的 `FIXREQ-v1.6.3.6-01` 工单号注释按规约**保留不改**
（工单号是历史标识，非版本标记）。
...
施工人：智能体 Q
施工对象：v1.6.3.7（FIXREQ-v1.6.3.6-01：UAT36-01 + D-04 终态收敛缺口与前端活动判定整改）
提交给：Mr.Linsang
