# TDSQL Copilot 专家助手详细设计说明书

|项目|内容|
|---|---|
|文档编号 / 修订|CP-DETAIL / Rev.A|
|编制日期 / 设计责任|2026-09-12 / O|
|提交对象|Mr.Linsang|
|开发状态|仅设计，待独立评审；本次不修改代码、配置、迁移或业务数据|
|发布版本|待 Mr.Linsang 指定；CP-1 是能力阶段名，不是发布版本号|
|代码基线|TDSQL `b6ce21bbcd731fe4f541c6ec09cd7a1316e52de3`，APP_VERSION=1.6.3.7|
|参考实现|DB-AIOps `a4dfb3b8d102c2fd9af4a37f416c273aa14840e1`，本地与 GitHub master 一致|
|上游文档|[需求分析与规划](PLAN-AI-Copilot-需求分析与总体规划-O.md)、[高阶设计](HLD-AI-Copilot-专家助手高阶设计-O.md)|

本文件是 CP-1 的施工规格。“新增/修改/必须”描述**未来开发要求**，不是已完成事实。Q 不得将概念描述、模型自述、mock通过或本设计默认参数当成实机验收结果。
...
|门禁|责任材料|阻断条件|
|---|---|---|
|CP-GATE-DESIGN|A设计评审、O整改闭环、Mr.Linsang版本决定|P0/P1未关闭、协议/授权范围仍含歧义|
...
开工前必须核定发布版本、确认设计评审、重查基线及v16占用、冻结API/schema/权限/错误闭集；实施顺序按§14。上线前再取得模型/data-zone/实例授权/保留条件及§16门禁证据。默认关闭、只读建议、无隐式公网回退、原审核权威、180秒既有统计预算不变，均不得在实现中悄悄放宽。
