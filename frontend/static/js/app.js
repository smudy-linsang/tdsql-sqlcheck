...
const app=createApp({
  setup(){
    ...
    const auditSql=async()=>{if(!sqlInput.value.trim())return;auditing.value=true;try{
      // V1.4：尺度由管理员全局启用，不再传 project_id（传了也会被忽略并返回 deprecated 提示）
      const body={sql:sqlInput.value};const resp=await apiFetch(`${API_BASE}/api/v1/audit/sql`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});auditResult.value=await resp.json()}catch(e){ElementPlus.ElMessage.error('审核请求失败: '+e.message)}finally{auditing.value=false}};
    ...
    const onFileChange=async(file)=>{if(!file||!file.raw)return;const content=await file.raw.text();try{
      // V1.4：尺度全局化，不再传 project_id（文档 §7.4 明确要求移除该传参）
      const body={content,file_path:file.name};const resp=await apiFetch(`${API_BASE}/api/v1/audit/file`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});fileAuditResult.value=await resp.json();ElementPlus.ElMessage.success('文件审核完成')}catch(e){ElementPlus.ElMessage.error('文件审核失败: '+e.message)}};
