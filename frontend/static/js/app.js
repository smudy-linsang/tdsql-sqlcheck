...
const app=createApp({
  setup(){
    ...
    const deleteConn=async(row)=>{try{await ElementPlus.ElMessageBox.confirm(`确认删除连接「${row.name}」？`,'删除确认',{type:'warning'})}catch(e){return}try{await apiFetch(`${API_BASE}/api/v1/tdsql/connections/${row.id}`,{method:'DELETE'});ElementPlus.ElMessage.success('已删除');loadSavedConnections()}catch(e){ElementPlus.ElMessage.error('删除失败')}};
        // V1.5：手动探测实例类型（探测优先于声明，冲突时提示核实）
        const probeInstanceType=async(row)=>{try{const resp=await apiFetch(`${API_BASE}/api/v1/tdsql/connections/${row.id}/probe-instance-type`,{method:'POST'});const d=await resp.json();if(resp.ok){ElementPlus.ElMessageBox.alert(d.message||'探测完成','实例类型探测结果',{type:d.conflict?'warning':'success'});loadSavedConnections()}else{ElementPlus.ElMessage.error((d.detail&&d.detail.detail)||d.detail||'探测失败')}}catch(e){ElementPlus.ElMessage.error('探测失败: '+e.message)}};
