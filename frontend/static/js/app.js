...
const app=createApp({
  setup(){
    ...
    const slowFilters=reactive({db_name:'',set_id:'',severity:'',status:'',keyword:'',scan_task_id:''});
    ...
    const toggleScheduleEnabled=async(row)=>{try{await apiFetch(`${API_BASE}/api/v1/tdsql/scan-schedules/${row.id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled:row.enabled?0:1})});ElementPlus.ElMessage.success('已更新');loadScanSchedules()}catch(e){ElementPlus.ElMessage.error('更新失败')}};
    // P0-01: 数据库体检
    ...
    // P0-01: 操作审计日志
    const loadAuditLogs=async()=>{auditLogsLoading.value=true;try{const offset=(auditLogsPage.value-1)*20;const resp=await apiFetch(`${API_BASE}/api/v1/admin/operation-logs?limit=20&offset=${offset}`);if(resp.ok){const d=await resp.json();auditLogs.value=d.items||d.logs||[];auditLogsTotal.value=d.total||0}}catch(e){}finally{auditLogsLoading.value=false}};
    // P0-01: 数据保留
    ...
    const createInspection=async()=>{if(!inspectionDialog.form.connection_id){ElementPlus.ElMessage.warning('请选择实例');return}inspectionDialog.loading=true;try{const resp=await apiFetch(`${API_BASE}/api/v1/inspection/tasks`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(inspectionDialog.form)});if(resp.ok){ElementPlus.ElMessage.success('巡检任务已创建');inspectionDialog.visible=false;loadInspectionTasks()}else{const d=await resp.json();ElementPlus.ElMessage.error(d.detail||'创建失败')}}catch(e){ElementPlus.ElMessage.error('创建失败: '+e.message)}finally{inspectionDialog.loading=false}};
