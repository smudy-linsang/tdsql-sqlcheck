...
// v1.6.1.0：预检失败码可读标签（设计 DESIGN-v1.6.0.8 §3.2）
function zkFailureLabel(code){
  return ({NO_BUSINESS_USER:'未创建监控用户',NO_AVAILABLE_PROXY:'Proxy 不可达',BUSINESS_PROXY_TIMEOUT:'枚举超时'})[code]||code||'失败';
}
...
const app=createApp({
  setup(){
    ...
    const savedConnections=connectionOptions;
    const managedConnections=ref([]);
    ...
    const fileAuditInstType=ref('distributed');
    const slowTasksTab=ref('tasks');
    const bigtableTab=ref('inventory');
    ...
    const selectedFileReportIds=ref([]);
    const fileReportsDeleting=ref(false);
    const fileReportsTableRef=ref(null);
    const fileReportFilters=reactive({keyword:'',instance_type:'',status:'',date_range:[]});
    const resetFileReportFilters=()=>{fileReportFilters.keyword='';fileReportFilters.instance_type='';fileReportFilters.status='';fileReportFilters.date_range=[];fileReportsPage.value=1;loadFileReports()};
    const rulesList=ref([]);
    ...
    // V1.5.3 原始慢日志：独立状态域，不与扫描任务/慢SQL记录混用。
    const rawSlowTab=ref('events');
    const rawEvents=ref([]);
    const rawEventsTotal=ref(0);
    const rawEventsLoading=ref(false);
    const rawRuns=ref([]);
    const rawRunsLoading=ref(false);
    const rawSources=ref([]);
    const rawSourcesLoading=ref(false);
    const rawFilters=reactive({source_id:'',db_name:'',time_range:[],min_query_time_ms:''});
    const rawSourceDrawer=ref(false);
    const rawSourceSaving=ref(false);
    const rawSourceEditMode=ref(false);
    const rawSourceForm=reactive({source_key:'',connection_id:'',display_name:'',transport:'ssh_exporter_v1',timezone:'Asia/Shanghai',poll_interval_seconds:60,max_batch_bytes:8388608,max_events_per_batch:2000,max_run_seconds:25,lag_alert_seconds:600,initial_position:'tail',initial_lookback_seconds:300,min_query_time_ms:1000,credential_ref:'',known_hosts_ref:'',nodes:[{node_key:'proxy_a',display_name:'Proxy A',ssh_host:'',ssh_port:22,host_key_alias:'',remote_source_key:'',declared_path_template:'',parser_profile:'tdsql_mysql_slowlog_v1'}]});
    const explainMode=ref('sql');
    const explainSqlInput=ref('');
    const explainInput=ref('');
    const explainConnId=ref('');
        // v1.6.2.2-UAT-O-08：EXPLAIN 库名是独立可见的表单字段，不再从 slowDetail 隐式继承
        const explainDbName=ref('');
    ...
    const connForm=reactive({id:'',name:'',host:'',port:3306,username:'',password:'',database:'',is_distributed:true,description:'',set_list:'',monitor_host:'',monitor_port:15001,monitor_user:'',monitor_password:'',monitor_db:'tdsqlpcloud_monitor',max_error_count:0,max_warning_count:-1});
    // V1.4：记录打开抽屉时的门禁阈值，用于判断保存时是否需要单独提交门禁（仅 admin）
    const gateOrig=ref({e:0,w:-1});
    const connEditMode=ref(false);
    const connTestResult=ref(null);
    const connTesting=ref(false);
    const connLoading=ref(false);
        // v1.6.1.1：实例管理客户端分页 + 三维筛选（设计 DESIGN-v1.6.1.1 §2，复用 ZK 发现列表模式）
        const connFilters=reactive({name:'',address:'',type:''});
        const connPage=ref(1);
        const connPageSize=ref(20);
        const filteredConnections=computed(()=>{
          const kw=(connFilters.name||'').trim().toLowerCase();
          const aw=(connFilters.address||'').trim().toLowerCase();
          const tp=connFilters.type||'';
          return (managedConnections.value||[]).filter(c=>{
            if(kw&&!(c.name||'').toLowerCase().includes(kw))return false;
            const addr=(c.host||'')+':'+(c.port||'')+(c.database?'/'+c.database:'');
            if(aw&&!addr.toLowerCase().includes(aw))return false;
            if(tp&&(c.effective_instance_type||'distributed')!==tp)return false;
            return true;
          });
        });
        ...
        const onConnFilterChange=()=>{connPage.value=1;};
    const usersList=ref([]);
    ...
    // v1.6.3.5 / DU-2：在线元数据审核任务（异步任务接口）
    const metadataJob=ref(null);          // 当前任务状态对象
    const metadataItems=ref([]);          // 当前页结果条目
    const metadataResultsTotal=ref(0);
    ...
    // G10: ZK 自动发现与管理员可维护的运行配置
    const zkDialogVisible=ref(false);
    const zkConfigDialogVisible=ref(false);
    ...
    const zkImportCommitting=ref(false);
    const zkImportPreviewId=ref('');
    const zkImportRows=ref([]);
    const zkImportSelected=ref([]);
    const zkImportSummary=ref({selected_instances:0,ready:0,conflict:0,error:0});
    const zkImportTableRef=ref(null);
    const zkDiscoveryTableRef=ref(null);
    const zkImportForm=reactive({
    ...
    const gatewayHtml=ref('');
        // v1.6.2.2-UAT-O-10：报告改用独立文档响应（iframe src）加载，srcdoc 会继承主文档 CSP 拦截内联脚本
        const gatewayReportUrl=ref('');
    ...
    // v1.6.2.2-UAT-O-16：巡检比对结果绑定 {connection_id,date1,date2,generated_at,status}，
    // 模板仅在结果范围与当前选择完全一致且状态成功时才显示，杜绝旧结果冒充本次结果。
    const dailyCompareScope=ref(null);
    const dailyInstSearch=ref('');
    const dailyInstSigOnly=ref(false);
    const dailyInstNodeSelect=ref('');
    const dailyInstPage=ref(1);
    const dailyInstPageSize=ref(15);
    const dailySrvSearch=ref('');
    const dailySrvSigOnly=ref(false);
    const dailySrvIpSelect=ref('');
    const dailySrvPage=ref(1);
    const dailySrvPageSize=ref(15);
    ...
    const roleLabel=computed(()=>({admin:'系统管理员',dba:'DBA',developer:'开发',auditor:'审计员'}[authState.role]||''));
        const roleLabelFn=(r)=>{if(rolesList.value&&rolesList.value.length){const found=rolesList.value.find(item=>item.role_id===r||item.role===r);if(found&&(found.role_name||found.label))return found.role_name||found.label}return {admin:'系统管理员',dba:'DBA',developer:'开发人员',auditor:'审计员'}[r]||r};
    const canManagePlatform=computed(()=>['admin','dba'].includes(authState.role));
    // V1.4：门禁阈值修改与全局规则集切换是治理动作，仅 admin 可操作
    ...
    const breadcrumbItems=computed(()=>{const m={dashboard:[{key:'d',label:'工作台'},{key:'c',label:'治理概览'}],'audit-sql':[{key:'a',label:'SQL审核'},{key:'c',label:'即时审核'}],'file-audit':[{key:'a',label:'SQL审核'},{key:'c',label:'文件审核'}],'schema-extractor-audit':[{key:'a',label:'SQL审核'},{key:'c',label:'在线元数据审核'}],rules:[{key:'p',label:'平台治理'},{key:'c',label:'审核规则库'}],'slow-tasks':[{key:'s',label:'慢SQL治理'},{key:'c',label:'扫描任务'}],'slow-records':[{key:'s',label:'慢SQL治理'},{key:'c',label:'慢SQL记录'}],'slow-raw-log':[{key:'s',label:'慢SQL治理'},{key:'c',label:'原始慢日志'}],'slow-schedule':[{key:'s',label:'慢SQL治理'},{key:'c',label:'扫描计划'}],explain:[{key:'s',label:'慢SQL治理'},{key:'c',label:'EXPLAIN分析'}],instances:[{key:'p',label:'平台治理'},{key:'c',label:'实例管理'}],'schema-check':[{key:'i',label:'实例体检'},{key:'c',label:'上线检查'}],bigtable:[{key:'i',label:'实例体检'},{key:'c',label:'大表治理'}],projects:[{key:'p',label:'平台治理'},{key:'c',label:'项目管理'}],rulesets:[{key:'p',label:'平台治理'},{key:'c',label:'规则集'}],gate:[{key:'p',label:'平台治理'},{key:'c',label:'质量门禁'}],monitor:[{key:'p',label:'平台治理'},{key:'c',label:'监控告警'}],inspection:[{key:'p',label:'平台治理'},{key:'c',label:'巡检管理'}],'sys-users':[{key:'sys',label:'系统管理'},{key:'c',label:'用户管理'}],'sys-retention':[{key:'sys',label:'系统管理'},{key:'c',label:'数据保留'}],'sys-auditlog':[{key:'sys',label:'系统管理'},{key:'c',label:'操作审计'}],'sys-info':[{key:'sys',label:'系统管理'},{key:'c',label:'系统信息'}],'sys-roles':[{key:'sys',label:'系统管理'},{key:'c',label:'角色管理'}],'sys-perms':[{key:'sys',label:'系统管理'},{key:'c',label:'权限矩阵'}]};return m[currentPage.value]||[]});
    ...
    const sourceLabel=(s)=>({digest:'性能摘要快照',processlist:'进程快照',manual:'手动录入'}[s]||s);
    ...
    const clearRoleScopedState=()=>{
      ...
      // UAT-O-G14-01 ⑤：登录态彻底隔离——连接选择与深度诊断上下文一并清空，
      // 前一用户无权访问的实例与旧统计结果不得带给后一用户
      ...
      deepDb.value='';
      deepLoading.value='';
      deepResult.cluster=null;deepResult.index=null;deepResult.diff=null;deepResult.emergency=null;deepResult.sqlstats=null;deepResult.tabletype=null;
      tabletypeSeq.value+=1;
      tabletypeLoading.value=false;
      resetTableTypeState();
      try{localStorage.removeItem('tdsql_conn')}catch(e){}
    };
    const doLogin=async()=>{
    ...
    const auditSql=async()=>{if(!sqlInput.value.trim())return;auditing.value=true;try{
      const body={sql:sqlInput.value,instance_type:instantAuditInstType.value};const resp=await apiFetch(`${API_BASE}/api/v1/audit/sql`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});auditResult.value=await resp.json()}catch(e){ElementPlus.ElMessage.error('审核请求失败: '+e.message)}finally{auditing.value=false}};
    ...
    const onFileChange=async(file)=>{if(!file||!file.raw)return;const content=await file.raw.text();try{
      const body={content,file_path:file.name,instance_type:fileAuditInstType.value};const resp=await apiFetch(`${API_BASE}/api/v1/audit/file`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});if(resp.ok){fileAuditResult.value=await resp.json();ElementPlus.ElMessage.success('文件审核完成');loadFileReports()}else{const d=await resp.json();ElementPlus.ElMessage.error((d.message)||(d.detail&&d.detail.detail)||d.detail||'文件审核失败')}}catch(e){ElementPlus.ElMessage.error('文件审核失败: '+e.message)}};
    const loadFileReports=async()=>{fileReportsLoading.value=true;try{const offset=(fileReportsPage.value-1)*10;const p=new URLSearchParams({limit:10,offset});if(fileReportFilters.keyword)p.set('keyword',fileReportFilters.keyword);if(fileReportFilters.instance_type)p.set('instance_type',fileReportFilters.instance_type);if(fileReportFilters.status)p.set('status',fileReportFilters.status);if(fileReportFilters.date_range&&fileReportFilters.date_range.length===2){p.set('start_date',fileReportFilters.date_range[0]);p.set('end_date',fileReportFilters.date_range[1])}const resp=await apiFetch(`${API_BASE}/api/v1/audit/file-reports?${p}`);if(resp.ok){const d=await resp.json();fileReports.value=d.items||[];fileReportsTotal.value=d.total||0}}catch(e){}finally{fileReportsLoading.value=false}};
    ...
    const onFileReportsSelect=(rows)=>{selectedFileReportIds.value=(rows||[]).map(r=>r.id)};
    const batchDeleteFileReports=async()=>{if(!selectedFileReportIds.value.length)return;try{await ElementPlus.ElMessageBox.confirm(`确认批量删除已选中的 ${selectedFileReportIds.value.length} 条文件审核历史记录？`,'批量删除确认',{type:'warning'})}catch(e){return}fileReportsDeleting.value=true;try{const resp=await apiFetch(`${API_BASE}/api/v1/audit/file-reports/batch-delete`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ids:selectedFileReportIds.value})});if(resp.ok){const d=await resp.json();ElementPlus.ElMessage.success(`成功删除 ${d.deleted||0} 条记录`);selectedFileReportIds.value=[];if(fileReportsTableRef.value)fileReportsTableRef.value.clearSelection();loadFileReports()}else{const d=await resp.json();ElementPlus.ElMessage.error((d.detail&&d.detail.detail)||d.detail||'删除失败')}}catch(e){ElementPlus.ElMessage.error('删除失败: '+e.message)}finally{fileReportsDeleting.value=false}};
    const deleteSingleFileReport=async(row)=>{try{await ElementPlus.ElMessageBox.confirm(`确认删除文件审核报告「${row.source}」 (ID: ${row.id})？`,'删除确认',{type:'warning'})}catch(e){return}try{const resp=await apiFetch(`${API_BASE}/api/v1/audit/file-reports/batch-delete`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ids:[row.id]})});if(resp.ok){ElementPlus.ElMessage.success('审核报告已删除');loadFileReports()}else{const d=await resp.json();ElementPlus.ElMessage.error((d.detail&&d.detail.detail)||d.detail||'删除失败')}}catch(e){ElementPlus.ElMessage.error('删除失败: '+e.message)}};
    const onSlowTasksTabChange=(t)=>{if(t==='tasks')loadScanTasks();else if(t==='compare')cmpQuery('slow_scan');else if(t==='reports')loadCompareReports('slow_scan')};
    const onBigtableTabChange=(t)=>{if(t==='inventory')loadBigtable();else if(t==='compare')cmpQuery('bigtable');else if(t==='reports')loadCompareReports('bigtable')};
    ...
    // v1.6.2.2-UAT-O-08：详情抽屉关闭时清除 slowDetail，防止库名等上下文残留串到其他页面
    const closeSlowDetail=()=>{slowDetailDrawer.value=false;slowDetail.value=null};
    ...
    const goSlowDetail=(r)=>{currentPage.value='slow-records';openSlowDetail(r)};
        // v1.6.2.2-UAT-O-08：仅显式「从该慢 SQL 去分析」时，才把来源上下文原子复制到 EXPLAIN 表单
        const goExplainFromSlow=(d)=>{currentPage.value='explain';explainMode.value='sql';explainSqlInput.value=d.sql_template||d.fingerprint||'';if(d.connection_id)explainConnId.value=d.connection_id;explainDbName.value=d.db_name||'';slowDetailDrawer.value=false;slowDetail.value=null};
    // ═══ V1.5.3 原始慢日志（独立 API / 独立时间语义）═══
    const rawApi='/api/v1/raw-slowlogs';
    const loadRawSources=async()=>{rawSourcesLoading.value=true;try{const r=await apiFetch(`${API_BASE}${rawApi}/sources`);if(r.ok){const d=await r.json();rawSources.value=d.items||[]}}catch(e){}finally{rawSourcesLoading.value=false}};
    const loadRawRuns=async()=>{rawRunsLoading.value=true;try{const r=await apiFetch(`${API_BASE}${rawApi}/runs?limit=100`);if(r.ok){const d=await r.json();rawRuns.value=d.items||[]}}catch(e){}finally{rawRunsLoading.value=false}};
    const loadRawEvents=async()=>{rawEventsLoading.value=true;try{const p=new URLSearchParams({limit:'200'});if(rawFilters.source_id)p.set('source_id',rawFilters.source_id);if(rawFilters.db_name)p.set('db_name',rawFilters.db_name);if(rawFilters.time_range&&rawFilters.time_range.length===2){p.set('start_time',rawFilters.time_range[0]);p.set('end_time',rawFilters.time_range[1])}if(rawFilters.min_query_time_ms!==''&&rawFilters.min_query_time_ms!==null)p.set('min_query_time_us',String(Math.round(Number(rawFilters.min_query_time_ms)*1000)));const r=await apiFetch(`${API_BASE}${rawApi}/events?${p}`);if(r.ok){const d=await r.json();rawEvents.value=d.items||[];rawEventsTotal.value=d.total||0}}catch(e){}finally{rawEventsLoading.value=false}};
    const resetRawFilters=()=>{rawFilters.source_id='';rawFilters.db_name='';rawFilters.time_range=[];rawFilters.min_query_time_ms='';loadRawEvents()};
    const downloadRawEvents=async(format='csv')=>{try{const p=new URLSearchParams({format});if(rawFilters.source_id)p.set('source_id',rawFilters.source_id);if(rawFilters.db_name)p.set('db_name',rawFilters.db_name);if(rawFilters.time_range&&rawFilters.time_range.length===2){p.set('start_time',rawFilters.time_range[0]);p.set('end_time',rawFilters.time_range[1])}if(rawFilters.min_query_time_ms!==''&&rawFilters.min_query_time_ms!==null)p.set('min_query_time_us',String(Math.round(Number(rawFilters.min_query_time_ms)*1000)));const r=await apiFetch(`${API_BASE}${rawApi}/events/export?${p}`);if(!r.ok){const d=await r.json().catch(()=>({}));throw new Error(d.detail||'导出失败')}const url=URL.createObjectURL(await r.blob());const a=document.createElement('a');a.href=url;a.download=`raw-slowlog-events.${format==='html'?'html':'csv'}`;a.click();URL.revokeObjectURL(url)}catch(e){ElementPlus.ElMessage.error(e.message||'导出失败')}};
    const resetRawSourceForm=()=>{Object.assign(rawSourceForm,{id:'',source_key:'',connection_id:'',display_name:'',transport:'ssh_exporter_v1',timezone:'Asia/Shanghai',poll_interval_seconds:60,max_batch_bytes:8388608,max_events_per_batch:2000,max_run_seconds:25,lag_alert_seconds:600,initial_position:'tail',initial_lookback_seconds:300,min_query_time_ms:1000,credential_ref:'',known_hosts_ref:'',nodes:[{node_key:'proxy_a',display_name:'Proxy A',ssh_host:'',ssh_port:22,host_key_alias:'',remote_source_key:'',declared_path_template:'',parser_profile:'tdsql_mysql_slowlog_v1'}]})};
    const openRawSourceCreate=()=>{resetRawSourceForm();rawSourceEditMode.value=false;rawSourceDrawer.value=true};
    const openRawSourceEdit=async(row)=>{try{const r=await apiFetch(`${API_BASE}${rawApi}/sources/${row.id}`);if(!r.ok)throw new Error('读取配置失败');const d=await r.json();Object.assign(rawSourceForm,{...d,nodes:(d.nodes||[]).map(n=>({node_key:n.node_key,display_name:n.display_name,ssh_host:n.ssh_host,ssh_port:n.ssh_port,host_key_alias:n.host_key_alias,remote_source_key:n.remote_source_key,declared_path_template:n.declared_path_template,parser_profile:n.parser_profile}))});rawSourceEditMode.value=true;rawSourceDrawer.value=true}catch(e){ElementPlus.ElMessage.error(e.message||'读取配置失败')}};
    const addRawNode=()=>rawSourceForm.nodes.push({node_key:'proxy_'+String.fromCharCode(97+rawSourceForm.nodes.length),display_name:'',ssh_host:'',ssh_port:22,host_key_alias:'',remote_source_key:rawSourceForm.source_key,declared_path_template:'',parser_profile:'tdsql_mysql_slowlog_v1'});
    const removeRawNode=(i)=>{if(rawSourceForm.nodes.length>1)rawSourceForm.nodes.splice(i,1)};
    const saveRawSource=async()=>{if(!rawSourceForm.source_key||!rawSourceForm.connection_id||!rawSourceForm.display_name){ElementPlus.ElMessage.warning('请填写源键、关联实例和展示名称');return}rawSourceSaving.value=true;try{const method=rawSourceEditMode.value?'PUT':'POST';const id=rawSourceForm.id?`/${rawSourceForm.id}`:'';const r=await apiFetch(`${API_BASE}${rawApi}/sources${id}`,{method,headers:{'Content-Type':'application/json'},body:JSON.stringify(rawSourceForm)});if(!r.ok){const d=await r.json().catch(()=>({}));throw new Error(d.detail||'保存失败')}ElementPlus.ElMessage.success(rawSourceEditMode.value?'配置已更新并自动停用':'已创建禁用采集源');rawSourceDrawer.value=false;loadRawSources()}catch(e){ElementPlus.ElMessage.error(e.message||'保存失败')}finally{rawSourceSaving.value=false}};
    const probeRawSource=async(row)=>{try{const r=await apiFetch(`${API_BASE}${rawApi}/sources/${row.id}/probe`,{method:'POST'});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.detail||'Probe失败');ElementPlus.ElMessage.success(d.status==='completed'?'Probe通过':'Probe完成，请检查运行详情');loadRawSources();loadRawRuns()}catch(e){ElementPlus.ElMessage.error(e.message||'Probe失败')}};
    const collectRawSource=async(row)=>{try{const r=await apiFetch(`${API_BASE}${rawApi}/sources/${row.id}/collect`,{method:'POST'});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.detail||'触发失败');ElementPlus.ElMessage.success(`已提交采集运行 #${d.run_id}`);setTimeout(()=>{loadRawRuns();loadRawEvents();loadRawSources()},800)}catch(e){ElementPlus.ElMessage.error(e.message||'触发失败')}};
    const toggleRawSource=async(row)=>{try{const action=row.enabled?'停用':'启用';await ElementPlus.ElMessageBox.confirm(`确认${action}采集源「${row.display_name}」？`,'确认',{type:'warning'});const r=await apiFetch(`${API_BASE}${rawApi}/sources/${row.id}/enabled`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled:!row.enabled})});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.detail||`${action}失败`);ElementPlus.ElMessage.success(`${action}成功`);loadRawSources()}catch(e){if(e!=='cancel'&&e!=='close')ElementPlus.ElMessage.error(e.message||'操作失败')}};
    const scanTaskFilters=reactive({connection_id:'',db_name:'',keyword:'',date_from:'',date_to:''});
    const scanTaskQuery=()=>{scanTaskCurrentPage.value=1;loadScanTasks()};
    const resetScanTaskFilters=()=>{scanTaskFilters.connection_id='';scanTaskFilters.db_name='';scanTaskFilters.keyword='';scanTaskFilters.date_from='';scanTaskFilters.date_to='';scanTaskQuery()};
    const loadScanTasks=async()=>{try{const offset=(scanTaskCurrentPage.value-1)*10;const f=scanTaskFilters;const p=new URLSearchParams({limit:10,offset,connection_id:f.connection_id||'',db_name:f.db_name||'',keyword:f.keyword||'',date_from:f.date_from||'',date_to:f.date_to||''});const resp=await apiFetch(`${API_BASE}/api/v1/slow-queries/scan-tasks?${p}`);if(resp.ok){const d=await resp.json();scanTasks.value=d.items||[];scanTaskTotal.value=d.total||0}}catch(e){}};
    ...
    const clearOrphanRecords=async()=>{try{await ElementPlus.ElMessageBox.confirm('确认清理所有无任务关联的慢SQL记录？此操作不可恢复。','清理确认',{type:'warning'})}catch(e){return}clearingOrphan.value=true;try{const resp=await apiFetch(`${API_BASE}/api/v1/slow-queries/orphan-records`,{method:'DELETE'});if(resp.ok){const d=await resp.json();ElementPlus.ElMessage.success(d.message||'清理完成');loadSlowList()}}catch(e){ElementPlus.ElMessage.error('清理失败')}finally{clearingOrphan.value=false}};
        // v1.6.2.2-UAT-O-08：db_name 只取自 EXPLAIN 页面自己的表单字段，不再读全局 slowDetail
        const analyzeExplainBySql=async()=>{if(!explainSqlInput.value.trim()||!explainConnId.value)return;analyzingExplain.value=true;explainResult.value=null;try{const body={sql:explainSqlInput.value,connection_id:explainConnId.value};if(explainDbName.value&&explainDbName.value.trim())body.db_name=explainDbName.value.trim();const resp=await apiFetch(`${API_BASE}/api/v1/slow-queries/analyze-explain-by-sql`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});if(!resp.ok){const err=await resp.json();throw new Error(err.detail||'EXPLAIN执行失败')}explainResult.value=await resp.json();ElementPlus.ElMessage.success('EXPLAIN分析完成')}catch(e){ElementPlus.ElMessage.error('分析失败: '+e.message)}finally{analyzingExplain.value=false}};
    ...
    const loadSavedConnections=loadManagedConnections;
    ...
    const testConn=async()=>{
      ...
      try{
        const body={
          ...
          is_distributed: connForm.is_distributed,
          monitor_host: connForm.monitor_host||'',
          monitor_port: connForm.monitor_port,
          monitor_user: connForm.monitor_user||'',
          monitor_password: connForm.monitor_password||'',
          monitor_db: connForm.monitor_db||'tdsqlpcloud_monitor'
        };
        ...
        const d=await resp.json();
        if(resp.ok&&d.status==='connected'){
          let msg=`业务库连接成功！${d.server_version}，延迟${d.latency_ms}ms。`;
          if(d.monitor_status==='connected'){
            msg+=` 监控库(monitordb)连接成功(发现 ${d.monitor_column_count} 列)。`;
            connTestResult.value={type:'success',msg:msg};
          }else if(d.monitor_status==='failed'){
            msg+=` ⚠️但监控库连接失败: ${d.monitor_error}`;
            connTestResult.value={type:'warning',msg:msg};
          }else{
            connTestResult.value={type:'success',msg:msg};
          }
        }else{
          let msg='业务库连接失败: '+(d.message||d.detail||'');
          if(d.monitor_error){msg+=`；监控库测试失败: ${d.monitor_error}`;}
          connTestResult.value={type:'error',msg:msg};
        }
      }catch(e){
        connTestResult.value={type:'error',msg:e.message};
      }finally{
        connTesting.value=false;
      }
    };
    const saveConn=async()=>{if(!connForm.name){ElementPlus.ElMessage.warning('请输入连接名称');return}if(connForm.is_distributed===null||connForm.is_distributed===undefined){ElementPlus.ElMessage.warning('请选择实例类型');return}
      // V1.4：门禁阈值是治理动作，不混入实例保存报文，实例保存成功后走独立 admin 接口
      const gateChanged=isAdmin.value&&(connForm.max_error_count!==gateOrig.value.e||connForm.max_warning_count!==gateOrig.value.w);
      try{const body={...connForm};delete body.max_error_count;delete body.max_warning_count;if(!body.id)delete body.id;let cid=connForm.id;
        if(connEditMode.value&&connForm.id){const resp=await apiFetch(`${API_BASE}/api/v1/tdsql/connections/${connForm.id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});if(!resp.ok){const d=await resp.json();ElementPlus.ElMessage.error(d.detail||'更新失败');return}ElementPlus.ElMessage.success('连接已更新')}
        else{const resp=await apiFetch(`${API_BASE}/api/v1/tdsql/connections`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});if(!resp.ok){const d=await resp.json();ElementPlus.ElMessage.error(d.detail||'保存失败');return}const saved=await resp.json().catch(()=>({}));cid=saved.id||cid;ElementPlus.ElMessage.success('连接已保存')}
        if(gateChanged&&cid){const gr=await apiFetch(`${API_BASE}/api/v1/gate/instances/${cid}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({max_error_count:connForm.max_error_count,max_warning_count:connForm.max_warning_count})});if(!gr.ok){const d=await gr.json().catch(()=>({}));const msg=(d.detail&&(d.detail.detail||d.detail))||'未知错误';ElementPlus.ElMessage.warning('实例已保存，但门禁阈值保存失败：'+msg)}}
        connDrawer.value=false;connEditMode.value=false;resetConnForm();await syncConnectionsAfterWrite();
      }catch(e){ElementPlus.ElMessage.error('保存失败: '+e.message)}};
    const resetConnForm=()=>{connForm.id='';connForm.name='';connForm.host='';connForm.port=3306;connForm.username='';connForm.password='';connForm.database='';connForm.is_distributed=true;connForm.description='';connForm.set_list='';connForm.monitor_host='';connForm.monitor_port=15001;connForm.monitor_user='';connForm.monitor_password='';connForm.monitor_db='tdsqlpcloud_monitor';connForm.max_error_count=0;connForm.max_warning_count=-1};
    // V1.5.1 F5：编辑抽屉里只读展示的 ZK 管控面信息
    const connZkInfo=ref({zk_instance_kind:'',zk_instance_id:'',zk_synced_at:''});
    const openEditConn=(row)=>{connEditMode.value=true;connZkInfo.value={zk_instance_kind:row.zk_instance_kind||'',zk_instance_id:row.zk_instance_id||'',zk_synced_at:row.zk_synced_at||''};connForm.id=row.id;connForm.name=row.name;connForm.host=row.host;connForm.port=row.port;connForm.username=row.username;connForm.password='';connForm.database=row.database||'';connForm.is_distributed=row.is_distributed!==0&&row.is_distributed!==false;connForm.description=row.description||'';connForm.set_list=row.set_list||'';connForm.monitor_host=row.monitor_host||'';connForm.monitor_port=row.monitor_port||15001;connForm.monitor_user=row.monitor_user||'';connForm.monitor_password='';connForm.monitor_db=row.monitor_db||'tdsqlpcloud_monitor';connForm.max_error_count=(row.max_error_count===undefined||row.max_error_count===null)?0:row.max_error_count;connForm.max_warning_count=(row.max_warning_count===undefined||row.max_warning_count===null)?-1:row.max_warning_count;gateOrig.value={e:connForm.max_error_count,w:connForm.max_warning_count};connTestResult.value=null;connDrawer.value=true};
    const openNewConn=()=>{connEditMode.value=false;resetConnForm();gateOrig.value={e:0,w:-1};connTestResult.value=null;connDrawer.value=true};
    const deleteConn=async(row)=>{
      try{await ElementPlus.ElMessageBox.confirm(`确认删除连接「${row.name}」？`,'删除确认',{type:'warning'})}catch(e){return}
      try{
        const resp=await apiFetch(`${API_BASE}/api/v1/tdsql/connections/${row.id}`,{method:'DELETE'});
        const d=await resp.json().catch(()=>({}));
        if(resp.ok){
          ElementPlus.ElMessage.success('已删除');
          await syncConnectionsAfterWrite();
        }else{
          ElementPlus.ElMessage.error(d.detail||'删除失败');
        }
      }catch(e){ElementPlus.ElMessage.error('删除失败: '+e.message)}
    };
        // V1.5.1：实例类型多源判定（锁定 > ZK > 探测/声明保守合并），弹窗改多源明细表
        const instTypeCn=(v)=>v==='distributed'?'分布式':(v==='centralized'?'集中式':'—');
        const instSourceCn=(s)=>({locked:'锁定',zk:'ZK',probed:'探测',declared:'声明',request:'请求',default:'默认'}[s]||s||'');
        const probeInstanceType=async(row)=>{try{const resp=await apiFetch(`${API_BASE}/api/v1/tdsql/connections/${row.id}/probe-instance-type`,{method:'POST'});const d=await resp.json();if(resp.ok){
          const s=d.sources||{};const esc=(x)=>String(x==null?'':x).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
          const tr=(label,src,extra)=>{src=src||{};return `<tr><td style="padding:4px 10px;border:1px solid #e5e7eb">${label}</td><td style="padding:4px 10px;border:1px solid #e5e7eb;text-align:center">${src.available?'✓':'✗'}</td><td style="padding:4px 10px;border:1px solid #e5e7eb">${esc(instTypeCn(src.value))}</td><td style="padding:4px 10px;border:1px solid #e5e7eb;color:#64748b">${esc(extra||src.reason||'')}</td></tr>`};
          const html=`<div style="font-size:13px"><table style="border-collapse:collapse;width:100%;margin-bottom:10px"><tr style="background:#f8fafc"><th style="padding:4px 10px;border:1px solid #e5e7eb">判定源</th><th style="padding:4px 10px;border:1px solid #e5e7eb">可用</th><th style="padding:4px 10px;border:1px solid #e5e7eb">结论</th><th style="padding:4px 10px;border:1px solid #e5e7eb">说明</th></tr>${tr('管理员锁定',s.locked,s.locked&&s.locked.available?'终审，覆盖一切自动源':'未锁定')}${tr('SQL 探测',s.probe)}${tr('ZK 管控面',s.zk,s.zk&&s.zk.kind?('形态 '+esc(s.zk.kind)):'')}${tr('实例声明',s.declared,'来自实例配置')}</table><div style="padding:6px 10px;background:${d.conflict?'#fef3c7':'#ecfdf5'};border-radius:6px"><b>最终生效：${esc(instTypeCn(d.effective_instance_type))}</b>（来源：${esc(instSourceCn(d.instance_type_source))}）</div><div style="margin-top:8px;color:#475569">${esc(d.message||'')}</div></div>`;
          ElementPlus.ElMessageBox.alert(html,'实例类型判定明细',{dangerouslyUseHTMLString:true,customStyle:{maxWidth:'640px'},type:d.conflict?'warning':'success'});await syncConnectionsAfterWrite()}else{ElementPlus.ElMessage.error((d.detail&&d.detail.detail)||d.detail||'探测失败')}}catch(e){ElementPlus.ElMessage.error('探测失败: '+e.message)}};
        // V1.5.1 F4：管理员锁定实例类型（仅 admin；锁集中式强制填理由）
        const lockDialog=reactive({visible:false,row:null,locked:true,instance_type:'centralized',reason:'',saving:false});
        const openLockDialog=(row)=>{lockDialog.row=row;lockDialog.locked=!row.instance_type_locked;lockDialog.instance_type=row.instance_type_locked_value||row.effective_instance_type||'centralized';lockDialog.reason='';lockDialog.visible=true};
        const submitLock=async()=>{const row=lockDialog.row;if(!row)return;
          if(lockDialog.locked&&lockDialog.instance_type==='centralized'&&!lockDialog.reason.trim()){ElementPlus.ElMessage.warning('锁定为「集中式」将跳过 27 条仅分布式适用的规则，请填写锁定理由');return}
          lockDialog.saving=true;
          try{const resp=await apiFetch(`${API_BASE}/api/v1/tdsql/connections/${row.id}/instance-type-lock`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({locked:lockDialog.locked,instance_type:lockDialog.instance_type,reason:lockDialog.reason.trim()})});const d=await resp.json();
            if(resp.ok){ElementPlus.ElMessage.success(d.message||'操作成功');lockDialog.visible=false;await syncConnectionsAfterWrite()}else{ElementPlus.ElMessage.error((d.detail&&d.detail.detail)||d.detail||'操作失败')}
          }catch(e){ElementPlus.ElMessage.error('操作失败: '+e.message)}finally{lockDialog.saving=false}};
        // V1.5.1 C.4：采集探测诊断（系统自身连接采集，采集环境=判定环境）
        const diagDialog=reactive({visible:false,loading:false,row:null,data:null,sampleTable:''});
        const openDiagDialog=(row)=>{diagDialog.row=row;diagDialog.data=null;diagDialog.sampleTable='';diagDialog.visible=true};
        const runDiagnostics=async()=>{const row=diagDialog.row;if(!row)return;diagDialog.loading=true;
          try{const resp=await apiFetch(`${API_BASE}/api/v1/tdsql/connections/${row.id}/probe-diagnostics`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sample_table:diagDialog.sampleTable.trim()})});const d=await resp.json();
            if(resp.ok){diagDialog.data=d;ElementPlus.ElMessage.success('诊断采集完成')}else{ElementPlus.ElMessage.error((d.detail&&d.detail.detail)||d.detail||'采集失败')}
          }catch(e){ElementPlus.ElMessage.error('采集失败: '+e.message)}finally{diagDialog.loading=false}};
        const downloadDiagnostics=()=>{if(!diagDialog.data)return;const blob=new Blob([JSON.stringify(diagDialog.data,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`probe_diag_${(diagDialog.data.endpoint||'inst').replace(/[:.]/g,'_')}_${new Date().toISOString().slice(0,19).replace(/[-:T]/g,'')}.json`;a.click();URL.revokeObjectURL(a.href)};
    const setDefaultConn=async(row)=>{
      try{
        const resp=await apiFetch(`${API_BASE}/api/v1/tdsql/connections/${row.id}/set-default`,{method:'POST'});
        const d=await resp.json().catch(()=>({}));
        if(resp.ok){
          ElementPlus.ElMessage.success('已设为默认');
          await syncConnectionsAfterWrite();
        }else{
          ElementPlus.ElMessage.error(d.detail||'设置失败');
        }
      }catch(e){ElementPlus.ElMessage.error('设置失败: '+e.message)}
    };
    const connectInstance=async(row)=>{
      row.connecting=true;
      try{
        const resp=await apiFetch(`${API_BASE}/api/v1/tdsql/connections/${row.id}/connect`,{method:'POST'});
        const d=await resp.json().catch(()=>({}));
        if(resp.ok){
          ElementPlus.ElMessage.success('已连接');
          await syncConnectionsAfterWrite();
        }else{
          ElementPlus.ElMessage.error(d.detail||'连接失败');
        }
      }catch(e){ElementPlus.ElMessage.error('连接失败: '+e.message)}finally{row.connecting=false}
    };
    const loadUsers=async()=>{
      if(!rolesList.value||!rolesList.value.length)loadRoles();
      usersLoading.value=true;
      try{
        const p=new URLSearchParams();
        p.set('limit',usersPageSize);
        p.set('offset',(usersPage.value-1)*usersPageSize);
        if(userKeyword.value)p.set('keyword',userKeyword.value);
        const resp=await apiFetch(`${API_BASE}/api/v1/auth/users?${p.toString()}`);
        if(resp.ok){const d=await resp.json();usersList.value=d.users||[];usersTotal.value=d.total||0}
      }catch(e){ElementPlus.ElMessage.error('加载用户列表失败')}
      finally{usersLoading.value=false}
    };
    const openUserCreate=()=>{
      if(!rolesList.value||!rolesList.value.length)loadRoles();
      const defaultRole=rolesList.value&&rolesList.value.length?rolesList.value[0].role_id||rolesList.value[0].role:'developer';
      userDialog.form={username:'',display_name:'',role:defaultRole,password:''};
      userDialog.visible=true;
    };
    const userQuery=()=>{usersPage.value=1;loadUsers()};
    ...
    // V1.5.2：页面改 3 页签，快照 module='launch_check'（与大表治理同款）。
    // 切页签必须调 cmpResetResult()：cmpState 是多模块共用的单例，
    // 不重置会把上一个模块的对比结果显示在当前模块下。
    const schemaCheckTab=ref('inventory');
    const onSchemaCheckTabChange=(name)=>{cmpResetResult();if(name==='compare')cmpQuery('launch_check');else if(name==='reports')loadCompareReports('launch_check')};
    const runSchemaCheck=async()=>{
      ...
      try{
        ...
        if(resp.ok){
          ...
          // V1.5.2：快照失败仅轻提示，检查结果不受影响（G6）
          if(data.snapshot_error)ElementPlus.ElMessage.warning(`本次结果未能存入历史（${data.snapshot_error}），检查结果不受影响。`);
    ...
    // ── 深度诊断：通用 POST 助手 ──
    // UAT-O-G14-03：中间件 403 用的是 message 字段，此前只读 detail，
    // auditor 点击后只能看到兜底的"执行失败"。统一错误文案提取，禁止 [object Object]。
    const apiErrorMessage=(data,fallback)=>{
      if(data==null)return fallback;
      const d=data.detail;
      if(typeof d==='string'&&d)return d;
      if(typeof data.message==='string'&&data.message)return data.message;
      if(Array.isArray(d)&&d.length){const m=d[0]&&d[0].msg;return m||fallback}
      return fallback;
    };
    const _deepPost=async(key,url,payload)=>{
      ...
      try{
        ...
        ElementPlus.ElMessage.error(apiErrorMessage(d,'执行失败'));return null;
    ...
    // UAT-O-G14-01：在途请求序号——迟到响应的作废凭据
    const tabletypeSeq=ref(0);
    // UAT2-O-G14-01：G14 独立 loading——deepLoading 是全深度诊断共用串，
    // 无法区分"旧请求 finally"与"新请求"的所有权；独立 ref 配合序号守卫。
    const tabletypeLoading=ref(false);
    // UAT-O-G14-01 ①：统一清理点。删掉任何一行，旧结果/旧历史都可能残留到下一次查询。
    const resetTableTypeState=()=>{
      deepResult.tabletype=null;
      tabletypeWarnAll.value=false;
      tabletypeHistoryVisible.value=false;
      tabletypeHistory.value=[];
      tabletypeDetailItems.value=[];
      tabletypeDetailAll.value=[];
      tabletypeDetailExpand.value=false;
      tabletypeDetailLoading.value=false;
    };
    ...
    // UAT-O-G14-01 ⑥：展示绑定范围——结果携带的（用户/实例/库）与当前上下文
    // 完全一致才渲染；否则视为残留，不显示。
    const _tabletypeScopeNow=()=>({
      username:(authState.user&&authState.user.username)||'',
      connectionId:deepConnId.value,
      database:(deepDb.value||'').trim(),
    });
    const tabletypeScopeMatch=computed(()=>{
      const r=deepResult.tabletype;
      if(!r||!r._scope)return false;
      const n=_tabletypeScopeNow();
      return r._scope.username===n.username&&r._scope.connectionId===n.connectionId&&r._scope.database===n.database;
    });
    const tabletypeView=computed(()=>tabletypeScopeMatch.value?deepResult.tabletype:null);
    const tabletypeScopeText=computed(()=>{
      const r=tabletypeView.value;
      if(!r)return '';
      const conn=(savedConnections.value.find(c=>c.id===r._scope.connectionId))||{};
      const name=conn.name?`${conn.name}（${conn.host||''}:${conn.port||''}）`:r._scope.connectionId;
      // UAT2-O-G14-02：缺失采集时间时明确提示，不得留下空白尾段
      const ts=r.created_at?formatTime(r.created_at):'采集时间不可用';
      return `${name} / ${r._scope.database||'全部业务库'} / ${ts}`;
    });
    const tabletypeWarnTotal=computed(()=>tabletypeView.value?tabletypeView.value.warnings.length:0);
    const visibleTableTypeWarnings=computed(()=>{
      const all=tabletypeView.value?tabletypeView.value.warnings:[];
      return tabletypeWarnAll.value?all:all.slice(0,TABLETYPE_WARN_CAP);
    });
    // UAT-O-G14-03：auditor 只读——按钮禁用并提示；后端 403 仍是最终防线，
    // 前端禁用只是体验层，不能依赖前端做授权。
    const canRunTableTypeStats=computed(()=>authState.role!=='auditor'&&visibleMenus.value.has('deep-diag-tabletype'));
    // UAT2-O-G14-01 ②：纯判定函数——该请求是否仍是当前上下文
    // （序号 + 登录用户 + 实例 + 规范化库名，任一漂移即过期）。
    const isTableTypeRequestCurrent=(mySeq,scope)=>{
      if(mySeq!==tabletypeSeq.value)return false;
      const n=_tabletypeScopeNow();
      return n.username===scope.username&&n.connectionId===scope.connectionId&&n.database===scope.database;
    };
    const runTableTypeStats=async()=>{
      // UAT2-O-G14-01 ③：不再委托 _deepPost——其内部在序号检查之前就会弹出
      // 错误提示，迟到请求的错误副作用无法撤回（切换实例后仍弹旧实例的
      // "实例连接失败"，用户会误判当前实例不可用）。本函数自带请求与提示，
      // 顺序固定：++seq → 快照 scope → 清空旧结果 → loading → 发请求 →
      // 回来先判 isCurrent，过期则静默丢弃（无提示/无数据/不动 loading）→
      // 仍当前才显示业务错误或写入结果 → finally 仅本请求仍持 loading 时关闭。
      tabletypeSeq.value+=1;
      const mySeq=tabletypeSeq.value;
      const scope=_tabletypeScopeNow();
      resetTableTypeState();
      tabletypeLoading.value=true;
      let r=null;
      try{
        const resp=await apiFetch(`${API_BASE}/api/v1/table-type-stats/run`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({connection_id:scope.connectionId,database:scope.database})});
        const d=await resp.json();
        if(!isTableTypeRequestCurrent(mySeq,scope))return;   // 过期：提示与数据全部作废
        if(!resp.ok){ElementPlus.ElMessage.error(apiErrorMessage(d,'执行失败'));return}
        r=d.data||d;
      }catch(e){
        if(!isTableTypeRequestCurrent(mySeq,scope))return;
        ElementPlus.ElMessage.error('执行失败: '+e.message);
        return;
      }finally{
        // UAT2-O-G14-01 ⑤：旧请求的 finally 不得误关新请求的 loading
        if(mySeq===tabletypeSeq.value)tabletypeLoading.value=false;
      }
      r._scope=scope;
    ...
    const openTableTypeHistory=async()=>{
      ...
      try{
        ...
        const d=await resp.json();
        if(!resp.ok){ElementPlus.ElMessage.error(apiErrorMessage(d,'加载历史失败'));return}
    ...
    const loadTableTypeHistoryDetail=async(row)=>{
      ...
      try{
        ...
        const d=await resp.json();
        if(!resp.ok){ElementPlus.ElMessage.error(apiErrorMessage(d,'加载明细失败'));return}
        const body=d.data||d;
    ...
    // v1.6.3.4 / D03：二级分区主表的诚实显示（DETAIL §4.4）。
    // 规则：UNKNOWN 显示 —（未知）；LEGACY 显示 —（历史未采集）；
    // NOT_APPLICABLE 显示 0（不适用）；COMPLETE 显示精确数；
    // PARTIAL 显示 ≥N（未完成），已确认 0 时显示"已确认 0，未完成"。
    // 严禁 value||0 抹掉 null——0 必须结合状态解读，只有 COMPLETE/0 才是真零。
    const fmtSecondaryMain=(row)=>{
      if(!row)return '—';
      const st=row.secondary_partition_check_state;
      const main=row.secondary_partition_main_tables;
      if(st==='LEGACY'||st===null||st===undefined)return '—（历史未采集）';
      if(st==='NOT_APPLICABLE')return '0（不适用）';
      if(st==='UNKNOWN')return '—（未知）';
      if(st==='COMPLETE')return String(main===null||main===undefined?0:main);
      if(st==='PARTIAL'){
        if(main===0||main===null||main===undefined)return '已确认 0，未完成';
        return '≥'+main+'（未完成）';
      }
      return String(main===null||main===undefined?'—':main);
    };
    // 实例汇总：在逐库显示基础上，inventory 不完整时追加"候选目录不完整"提示。
    const fmtSecondaryMainSummary=(view)=>{
      if(!view)return '—';
      const st=view.secondary_partition_check_state;
      const main=view.secondary_partition_main_tables;
      const inv=view.secondary_partition_inventory_state;
      if(st==='LEGACY'||st===null||st===undefined)return '—（历史未采集）';
      if(st==='NOT_APPLICABLE')return '0（不适用）';
      let base;
      if(st==='UNKNOWN')base='—（未知）';
      else if(st==='COMPLETE')base=String(main===null||main===undefined?0:main);
      else if(st==='PARTIAL')base=(main===0||main===null||main===undefined)?'已确认 0，未完成':('≥'+main+'（未完成）');
      else base=String(main===null||main===undefined?'—':main);
      if(inv==='PARTIAL'||inv==='FAILED')base+='（候选目录不完整，数量为已知范围）';
      return base;
    };
    // QC-DEFECT-03：二级分区主表指标的 Tooltip 释义（候选/已判明/未判明/未检查/非Proxy分片），
    // 让“≥N（未完成）”可解释——用户悬停即知为何未完成（哪个库没查完/哪类未判明）。
    const fmtSecondaryMainTooltip=(row)=>{
      if(!row)return '';
      const st=row.secondary_partition_check_state;
      if(st==='LEGACY'||st===null||st===undefined||st==='NOT_APPLICABLE')return '';
      const cand=row.secondary_partition_candidates;
      if(cand===null||cand===undefined)return '';
      const chk=row.secondary_partition_checked;
      const unk=row.secondary_partition_unknown;
      const unchk=row.secondary_partition_unchecked;
      const out=row.secondary_partition_outside_shard;
      const n=(v)=>(v===null||v===undefined?0:v);
      return `候选 ${cand} · 已判明 ${n(chk)} · 未判明 ${n(unk)} · 未检查 ${n(unchk)} · 非Proxy分片 ${n(out)}`;
    };
    // G10: ZK Discovery
    const applyZkConfig=(data={})=>{
      zkConfigForm.servers=data.servers||'';
      zkConfigForm.root_path=data.root_path||'/tdsqlzk';
      zkConfigForm.driver=data.driver||'kazoo';
      zkConfigForm.zkcli_path=data.zkcli_path||'';
      zkConfigForm.proxy_mode=data.proxy_mode||'first';
    ...
    const openZkConfig=async()=>{
      ...
      zkConfigLoading.value=true;
      try{
        const resp=await apiFetch(`${API_BASE}/api/v1/tdsql/discover/config`);
        const data=await resp.json().catch(()=>({}));
        if(!resp.ok){ElementPlus.ElMessage.error(responseMessage(data,'读取 ZK 发现配置失败'));return}
        applyZkConfig(data);
      }catch(e){ElementPlus.ElMessage.error('读取 ZK 发现配置失败: '+e.message)}
      finally{zkConfigLoading.value=false}
    };
    ...
    const saveZkConfig=async()=>{
      ...
      if(!zkConfigMeta.password_configured&&!zkConfigForm.auth_password){ElementPlus.ElMessage.warning('首次保存请填写 ZK 认证口令');return}
      const endpoint_map={};
      for(const row of zkEndpointRows.value){
      ...
      try{
        ...
        applyZkConfig(data);
        ElementPlus.ElMessage.success('ZK 发现配置已加密保存，后续扫描无需重启服务');
        zkConfigDialogVisible.value=false;
      }catch(e){ElementPlus.ElMessage.error('保存 ZK 发现配置失败: '+e.message)}
      finally{zkConfigSaving.value=false}
    };
    ...
    const runZkDiscovery=async()=>{
      ...
      try{
        ...
        if(resp.ok){
          ...
          const d=await resp.json().catch(()=>({}));
          ElementPlus.ElMessage.error(responseMessage(d,'扫描失败'));
        }
      }catch(e){
    ...
    // v1.6.0.5：关闭导入弹窗（未提交）时清空发现勾选与预览，避免下次预览混入旧选择。
    // 提交成功路径也会触发（dialog @close），此时清空属正常（选择已消费）。
    const closeZkImport=()=>{
      zkSelected.value=[];
      zkImportSelected.value=[];
      zkImportRows.value=[];
      zkImportPreviewId.value='';
      if(zkDiscoveryTableRef.value&&zkDiscoveryTableRef.value.clearSelection){
        zkDiscoveryTableRef.value.clearSelection();
      }
    };
    const runZkImportPreview=async()=>{
      if(!zkImportForm.business_username.trim()||!zkImportForm.business_password){ElementPlus.ElMessage.warning('请填写业务连接用户名和密码');return}
      if(!zkImportForm.monitor_host.trim()||!zkImportForm.monitor_port||!zkImportForm.monitor_username.trim()||!zkImportForm.monitor_password||!zkImportForm.monitor_database.trim()){
        ElementPlus.ElMessage.warning('请完整填写 MonitorDB 监控配置');return
      }
      ...
      try{
        ...
        const data=await resp.json().catch(()=>({}));
        // 页面不保留明文密码；服务端仅在五分钟预览会话内短暂保存以便确认提交。
        zkImportForm.business_password='';zkImportForm.monitor_password='';
    ...
    const commitZkImport=async()=>{
      ...
      zkImportCommitting.value=true;
      try{
        const resp=await apiFetch(`${API_BASE}/api/v1/tdsql/discover/import-commit`,{
          method:'POST',headers:{'Content-Type':'application/json'},
          body:JSON.stringify({discovery_id:zkDiscoveryId.value,preview_id:zkImportPreviewId.value,row_tokens:zkImportSelected.value.map(row=>row.row_token)})
        });
        const data=await resp.json().catch(()=>({}));
        if(!resp.ok){ElementPlus.ElMessage.error(responseMessage(data,'导入连接失败'));return}
        // P7：导入成功结果按批次展示明细（后端审计接口 GET /discover/import-batches/{id} 可追溯）
        const created=Array.isArray(data.connections)?data.connections:[];
        const lines=created.map(c=>`<div>• ${c.name}（库：${c.database}）</div>`).join('')||'<div>无</div>';
        ElementPlus.ElMessageBox.alert(
          `<div style="line-height:1.8">导入批次：<b>${data.batch_id||'-'}</b><br/>已创建 ${data.created_count||0} 条连接：<br/>${lines}<span style="color:#94a3b8;font-size:12px">批次审计可在服务端按 batch_id 追溯（zk_import_batch_id）。</span></div>`,
          'ZK 标准化导入完成',{dangerouslyUseHTMLString:true,confirmButtonText:'关闭'}).catch(()=>{});
        zkImportDialogVisible.value=false;
        zkDialogVisible.value=false;
        await syncConnectionsAfterWrite();
      }catch(e){ElementPlus.ElMessage.error('导入连接失败: '+e.message)}
      finally{zkImportCommitting.value=false}
    };
    ...
    const viewGatewayReport=async(row)=>{
      // v1.6.2.2-UAT-O-15：iframe 不再把长期登录令牌放进可见 URL；
      // 先用头部令牌经签发接口换取 90s 一次性报告票据，iframe 仅携带票据。
      ...
      try{
        ...
        gatewayReportUrl.value=`${API_BASE}/api/v1/gateway-log/reports/${row.id}/html?report_ticket=${encodeURIComponent(data.ticket)}`;
        gatewayHtml.value='';
      ...
      try{
        ...
        if(resp.ok){
          ...
          if(d.status==='partial'&&d.parse_quality){
            ...
            ElementPlus.ElMessage({type:'warning',duration:8000,showClose:true,
              message:`日志分析完成，但仅 ${q.parsed_lines}/${q.nonempty_lines} 行有效（覆盖率 ${cov}%，跳过 ${q.skipped_lines} 行）。报告结论仅覆盖已解析部分，详见报告顶部告警。`});
          }else{
    ...
    const resetDailyResult=()=>{
      dailyCompareResult.value=null;
      dailyCompareScope.value=null;
      dailyInspectChartData.value=null;
      dailyInspectChartNodes.value=[];
      dailyInspectChartNode.value='';
      dailyInstSearch.value='';
      dailyInstSigOnly.value=false;
      dailyInstNodeSelect.value='';
      dailyInstPage.value=1;
      dailySrvSearch.value='';
      dailySrvSigOnly.value=false;
      dailySrvIpSelect.value='';
      dailySrvPage.value=1;
    };
    // v1.6.2.2-UAT-O-16：只有结果范围与当前实例/日期完全一致且比对成功时才允许展示；
    // 切换实例、切换日期范围、全失败/比对失败后旧结果一律不可见。
    const dailyResultVisible=computed(()=>{
      const sc=dailyCompareScope.value;
      if(!sc||sc.status!=='success'||!dailyCompareResult.value)return false;
      const ds=dailyInspectDates.value||[];
      return sc.connection_id===deepConnId.value&&ds.length===2&&sc.date1===ds[0]&&sc.date2===ds[1];
    });
    ...
    const runDailyInspect = async () => {
      ...
      // v1.6.2.2-UAT-O-16：新任务开始即清空上一轮结果/趋势/筛选/分页，
      // 全失败、异常分支不再残留旧日期的比对结果。
      resetDailyResult();
      deepLoading.value = "daily_run";
      try {
        // v1.6.2.2-UAT-O-13：逐日检查响应状态与业务结果，不得无条件提示“已完成”——
        // apiFetch 返回 Response，4xx 不会 throw，曾导致 monitordb 不可用(400)时仍提示采集成功。
        const failed = [];
        for (const d of dates) {
          try {
            const resp = await apiFetch(`${API_BASE}/api/v1/daily-inspect/run`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
            ...
            if (!resp.ok) {
              ...
              try { const err = await resp.json(); detail = err.detail || detail; } catch (e2) {}
              failed.push(`${d}（${detail}）`);
            }
          } catch (e1) {
            failed.push(`${d}（${e1.message}）`);
          }
        }
        if (failed.length === dates.length) {
          ElementPlus.ElMessage.error("采集失败：" + failed.join("；") + "。请检查 MonitorDB 配置后重试");
          return;   // 全部失败：不触发比对，避免用空数据冒充“无差异”
        }
        if (failed.length > 0) {
          ElementPlus.ElMessage.warning(`部分日期采集失败：${failed.join("；")}；已成功 ${dates.length - failed.length} 天，正在生成比对分析大屏...`);
        } else {
    ...
    const compareDailyInspect = async () => {
      ...
      // v1.6.2.2-UAT-O-16：重新比对即先清空旧结果；比对失败/异常时保持清空状态。
      resetDailyResult();
      ...
      try {
        ...
        if (resp.ok) {
          ...
          // v1.6.2.2-UAT-O-16：结果绑定实例/日期/生成时刻/状态，模板按范围匹配后才展示。
          dailyCompareScope.value = {
            connection_id: deepConnId.value, date1: d1, date2: d2,
            generated_at: new Date().toISOString(), status: "success"
          };
          compareOk = true;
        } else {
          // v1.6.2.2-UAT-O-13：比对请求失败必须明示，不得用旧结果/空表冒充“比对完成”
          let detail = `HTTP ${resp.status}`;
          try { const err = await resp.json(); detail = err.detail || detail; } catch (e2) {}
          ElementPlus.ElMessage.error("差异比对失败: " + detail);
        }
        ...
        // v1.6.2.2-UAT-O-13：只有比对真的成功才提示完成
        if (compareOk) {
    ...
    // 实例级 & 服务器级 节点与 IP 下拉列表
    const dailyInstNodeList = computed(() => {
      if (!dailyCompareResult.value || !dailyCompareResult.value.instance_diffs) return [];
      const nodes = new Set();
      dailyCompareResult.value.instance_diffs.forEach(item => { if (item.node) nodes.add(item.node); });
      return Array.from(nodes);
    });

    const dailySrvIpList = computed(() => {
      if (!dailyCompareResult.value || !dailyCompareResult.value.server_diffs) return [];
      const ips = new Set();
      dailyCompareResult.value.server_diffs.forEach(item => { if (item.ip) ips.add(item.ip); });
    ...
    const filteredDailyInstDiffs = computed(() => {
      if (!dailyCompareResult.value || !dailyCompareResult.value.instance_diffs) return [];
      let list = dailyCompareResult.value.instance_diffs;
      if (dailyInstNodeSelect.value) {
        list = list.filter(item => item.node === dailyInstNodeSelect.value);
      }
      if (dailyInstSigOnly.value) {
        list = list.filter(item => !!item.significant);
      }
      if (dailyInstSearch.value) {
        const q = dailyInstSearch.value.toLowerCase().trim();
        list = list.filter(item => (item.node && item.node.toLowerCase().includes(q)) || (item.metric_label && item.metric_label.toLowerCase().includes(q)));
      }
      return list;
    });

    const pagedDailyInstDiffs = computed(() => {
      const start = (dailyInstPage.value - 1) * dailyInstPageSize.value;
      return filteredDailyInstDiffs.value.slice(start, start + dailyInstPageSize.value);
    });

    // 服务器级过滤与切片分页
    const filteredDailySrvDiffs = computed(() => {
      if (!dailyCompareResult.value || !dailyCompareResult.value.server_diffs) return [];
      let list = dailyCompareResult.value.server_diffs;
      if (dailySrvIpSelect.value) {
        list = list.filter(item => item.ip === dailySrvIpSelect.value);
      }
      if (dailySrvSigOnly.value) {
        list = list.filter(item => !!item.significant);
      }
      if (dailySrvSearch.value) {
        const q = dailySrvSearch.value.toLowerCase().trim();
        list = list.filter(item => (item.ip && item.ip.toLowerCase().includes(q)) || (item.hostname && item.hostname.toLowerCase().includes(q)) || (item.metric_label && item.metric_label.toLowerCase().includes(q)));
      }
      return list;
    });

    const pagedDailySrvDiffs = computed(() => {
      const start = (dailySrvPage.value - 1) * dailySrvPageSize.value;
      return filteredDailySrvDiffs.value.slice(start, start + dailySrvPageSize.value);
    });
    ...
    // D-03：submission 生命周期记录 {user,intent_id,key,job_id,submission_state}
    ...
    const runExtractAndAudit=async()=>{
      ...
      const key=_newMetaKey();  // 新扫描 = 新幂等键（intent_id=key）
      _saveSubmission({intent_id:key, key:key, job_id:null, submission_state:'pending'});
      try{
        const resp=await apiFetch(`${API_BASE}/api/v1/audit/metadata-jobs`,{
          method:'POST',
          headers:{'Content-Type':'application/json','Idempotency-Key':key},
          body:JSON.stringify({connection_id:extractedAuditConnId.value,database:extractedDbName.value,scopes:extractedScope.value,client_submission_key:key})
        });
        if(resp.status===410){ElementPlus.ElMessage.error('旧审核路径已退役，请刷新页面后重试');extractAuditing.value=false;return}
        const d=await resp.json().catch(()=>({}));
        if(resp.ok||resp.status===202){
          metadataJob.value=d;
          sessionStorage.setItem('meta_active_job',d.job_id);
          _saveSubmission({intent_id:key, key:key, job_id:d.job_id, submission_state:'accepted'});  // D-03
          ElementPlus.ElMessage.info('元数据审核任务已受理，正在后台执行…');
          _pollMetadataJob(d.job_id);
        }else if(resp.status===409&&d.code==='METADATA_BUSY'){
          ElementPlus.ElMessage.warning(d.message||'当前已有元数据审核任务，请稍后重试');extractAuditing.value=false;
        }else{
    ...
    const _pollMetadataJob=async(job_id)=>{
      const gen=++_metaPollGen;
      const tick=async()=>{
        if(gen!==_metaPollGen)return;  // 已被新任务/离开打断
        let d=null;
        try{
          const resp=await apiFetch(`${API_BASE}/api/v1/audit/metadata-jobs/${job_id}`);
          if(gen!==_metaPollGen)return;
          if(resp.status===401||resp.status===403){extractAuditing.value=false;return}  // 鉴权失败停止轮询
          if(resp.status===404){extractAuditing.value=false;return}
          if(resp.ok){d=await resp.json();}
        }catch(e){ if(gen===_metaPollGen){setTimeout(tick,4000);} return; }  // 网络错误退避重查
        if(!d){ if(gen===_metaPollGen)setTimeout(tick,2000); return; }
        metadataJob.value=d;
        if(d.state==='SUCCEEDED'||d.state==='FAILED'||d.state==='CANCELLED'){
          // D-03：终态后 submission 标记 closed（保留 job_id 供查看，下次点击必为新 key）
          const _sub=_loadSubmission(); if(_sub){_sub.submission_state='closed';_saveSubmission(_sub);}
        }
        if(d.state==='SUCCEEDED'){
          extractAuditing.value=false;
          await loadMetadataResults(job_id,1);
          const failed=(metadataResultsTotal.value&&d.error)?1:0;
          ElementPlus.ElMessage.success('元数据审核完成，已生成报告');
          loadExtractedReports();  // 刷新历史（行数只增一次）
        }else if(d.state==='FAILED'||d.state==='CANCELLED'){
          extractAuditing.value=false;
          ElementPlus.ElMessage.error((d.error&&d.error.message)||('任务'+d.state));
        }else if(d.state==='RECOVERY_REQUIRED'){
          extractAuditing.value=false;
          ElementPlus.ElMessage.error((d.error&&d.error.message)||'后台回收未完成，需运维处理');
        }else{
          if(gen===_metaPollGen)setTimeout(tick,2000);  // 运行中继续轮询
        }
      };
      tick();
    };
    const loadMetadataResults=async(job_id,page)=>{
      const gen=_metaPollGen;  // D-03：与轮询同源守卫，防止慢旧响应覆盖新视图
      metadataResultsPage.value=page||1;
      try{
        const offset=(metadataResultsPage.value-1)*metadataResultsPageSize.value;
        const resp=await apiFetch(`${API_BASE}/api/v1/audit/metadata-jobs/${job_id}/results?offset=${offset}&limit=${metadataResultsPageSize.value}`);
        if(gen!==_metaPollGen)return;  // generation 已变（新任务/离开），丢弃旧响应
        if(resp.ok){const d=await resp.json();metadataItems.value=d.items||[];metadataResultsTotal.value=d.total||0;}
      }catch(e){}
    };
    ...
    const cancelMetadataJob=async()=>{
      const job=metadataJob.value;
      if(!job||!job.job_id)return;
      try{
        await ElementPlus.ElMessageBox.confirm('会停止本次后台任务，尚未发布的结果不会作为完整报告。确认取消？','取消任务',{type:'warning',confirmButtonText:'确认取消',cancelButtonText:'再想想'});
      }catch(e){return}
      metadataCancelling.value=true;
      try{
        const resp=await apiFetch(`${API_BASE}/api/v1/audit/metadata-jobs/${job.job_id}/cancel`,{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
        if(resp.ok||resp.status===202){ElementPlus.ElMessage.info('取消及资源回收中…');}
        else{const d=await resp.json().catch(()=>({}));ElementPlus.ElMessage.warning(d.message||'取消请求已提交');}
      }catch(e){ElementPlus.ElMessage.error('取消请求失败: '+e.message)}
      finally{metadataCancelling.value=false}
    };
    const downloadMetadataSql=async()=>{
      const job=metadataJob.value;if(!job||!job.job_id)return;
      try{
        const resp=await apiFetch(`${API_BASE}/api/v1/audit/metadata-jobs/${job.job_id}/sql`);
        if(!resp.ok){ElementPlus.ElMessage.error('SQL 文件不可用');return}
        const blob=await resp.blob();const url=URL.createObjectURL(blob);
        const a=document.createElement('a');a.href=url;a.download=`metadata_${job.job_id}.sql`;
        document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);
        ElementPlus.ElMessage.success('已下载元数据 SQL 文件');
      }catch(e){ElementPlus.ElMessage.error('下载失败: '+e.message)}
    };
    const downloadMetadataHtml=async()=>{
      const job=metadataJob.value;if(!job||!job.job_id)return;
      try{
        const resp=await apiFetch(`${API_BASE}/api/v1/audit/metadata-jobs/${job.job_id}/html`);
        if(!resp.ok){ElementPlus.ElMessage.error('报告不可用');return}
        const blob=await resp.blob();const url=URL.createObjectURL(blob);
        const a=document.createElement('a');a.href=url;a.download=`metadata_report_${job.job_id}.html`;
        document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);
        ElementPlus.ElMessage.success('已下载 HTML 报告');
      }catch(e){ElementPlus.ElMessage.error('下载失败: '+e.message)}
    };
    ...
    const openRoleCreate=()=>{roleDialog.isEdit=false;roleDialog.form={role_id:'',role_name:'',description:''};roleDialog.visible=true};
    ...
    const saveRole=async()=>{
      ...
      try{
        if(roleDialog.isEdit){
          ...
          else{const d=await resp.json();const errMsg=Array.isArray(d.detail)?d.detail.map(e=>e.msg).join('; '):(d.detail||'更新失败');ElementPlus.ElMessage.error(errMsg)}
        }else{
          const payload={role_name:roleDialog.form.role_name,description:roleDialog.form.description};
          if(roleDialog.form.role_id&&roleDialog.form.role_id.trim()){payload.role_id=roleDialog.form.role_id.trim()}
          const resp=await apiFetch(`${API_BASE}/api/v1/auth/roles`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
          ...
          else{const d=await resp.json();const errMsg=Array.isArray(d.detail)?d.detail.map(e=>e.msg).join('; '):(d.detail||'创建失败');ElementPlus.ElMessage.error(errMsg)}
        }
    ...
    const deleteRuleset=async(row)=>{if(row.is_builtin){ElementPlus.ElMessage.warning('内置规则集不可删除');return}if(row.is_active){ElementPlus.ElMessage.warning('该规则集正在全局生效中，请先切换到其它规则集再删除');return}try{await ElementPlus.ElMessageBox.confirm(`确认删除规则集「${row.name}」？`,'删除确认',{type:'warning'})}catch(e){return}try{const resp=await apiFetch(`${API_BASE}/api/v1/rulesets/${row.id}`,{method:'DELETE'});if(resp.ok){ElementPlus.ElMessage.success('已删除');loadRulesets()}else{const d=await resp.json();ElementPlus.ElMessage.error((d.detail&&d.detail.detail)||d.detail||'删除失败')}}catch(e){ElementPlus.ElMessage.error('删除失败')}};
    // V1.4：切换全局生效规则集（admin）。确认文案必须写明影响面与 30 秒时延，避免管理员误判为故障
    const activateRuleset=async(row)=>{try{await ElementPlus.ElMessageBox.confirm(`确认将「${row.name}」设为全局生效规则集？系统所有审核与扫描将统一使用该尺度，最长 30 秒内全量生效。`,'切换生效规则集',{type:'warning'})}catch(e){return}try{const resp=await apiFetch(`${API_BASE}/api/v1/rulesets/${row.id}/activate`,{method:'POST'});if(resp.ok){const d=await resp.json();ElementPlus.ElMessage.success(d.message||'已切换生效规则集');loadRulesets()}else{const d=await resp.json();ElementPlus.ElMessage.error((d.detail&&d.detail.detail)||d.detail||'切换失败')}}catch(e){ElementPlus.ElMessage.error('切换失败: '+e.message)}};
    ...
    const runCompare=async()=>{
      ...
      try{
        const resp=await apiFetch(`${API_BASE}/api/v1/scan-compare/compare`,{method:'POST',
          headers:{'Content-Type':'application/json'},
          body:JSON.stringify({module:cmpState.module,snapshot_ids:cmpState.selected.map(s=>s.id)})});
        const data=await resp.json();
        if(!resp.ok){ElementPlus.ElMessage.error((data.detail&&data.detail.detail)||data.detail||'对比失败');return}
        cmpState.result=data;cmpState.visible=true;
      }catch(e){ElementPlus.ElMessage.error('对比失败: '+e.message)}
    ...
    const saveCompareReport=async()=>{
      if(cmpState.selected.length!==2){ElementPlus.ElMessage.warning('请先选择两次扫描结果');return}
      cmpState.saving=true;
      try{
        const resp=await apiFetch(`${API_BASE}/api/v1/scan-compare/reports`,{method:'POST',
          headers:{'Content-Type':'application/json'},
          body:JSON.stringify({module:cmpState.module,snapshot_ids:cmpState.selected.map(s=>s.id)})});
        const data=await resp.json();
    ...
    const cmpReportQuery=(module)=>{cmpState.reportsPage=1;loadCompareReports(module)};
    const cmpReportResetFilters=(module)=>{cmpState.reportFilters={connection_id:'',db_name:'',keyword:'',date_from:'',date_to:''};cmpReportQuery(module)};
    const loadCompareReports=async(module)=>{
      ...
      try{
        ...
        const rf=cmpState.reportFilters||{};
        const qs=new URLSearchParams({
          module:cmpState.module,limit:cmpState.reportsPageSize,offset,
          connection_id:rf.connection_id||'',db_name:rf.db_name||'',
          keyword:rf.keyword||'',date_from:rf.date_from||'',date_to:rf.date_to||''
        });
    ...
    const cmpFmtChange=(ch)=>{
      if(!ch)return '';
      const pct=(typeof ch.pct==='number')?`（${ch.pct>0?'+':''}${ch.pct}%）`:'';
    ...
    // UAT-O-G14-01 ④：实例或库名变化即失效旧结果，不等用户再点一次按钮；
    // 序号递增使在途响应作废，实例变化时还要关闭历史抽屉并清空历史/明细。
    watch([deepConnId,deepDb],(nv,ov)=>{
      tabletypeSeq.value+=1;
      tabletypeLoading.value=false;
      if(deepResult.tabletype)deepResult.tabletype=null;
      tabletypeWarnAll.value=false;
      if(nv[0]!==ov[0]){
        tabletypeHistoryVisible.value=false;
        tabletypeHistory.value=[];
    ...
    // ═══ V1.5.2 单次快照明细查看与 HTML 报告导出 ═══
    const snapshotDetailDialog=reactive({visible:false,loading:false,snapshot:null,search:'',severityFilter:''});
    const filteredSnapshotIssues=computed(()=>{
      const snap=snapshotDetailDialog.snapshot;
      if(!snap||!snap.issues) return [];
      let list=snap.issues;
      if(snapshotDetailDialog.severityFilter){
        list=list.filter(i=>(i.severity||'').toUpperCase()===snapshotDetailDialog.severityFilter.toUpperCase());
      }
      if(snapshotDetailDialog.search){
        const q=snapshotDetailDialog.search.toLowerCase();
        list=list.filter(i=>(i.title||'').toLowerCase().includes(q)||(i.object_name||'').toLowerCase().includes(q)||(i.detail||'').toLowerCase().includes(q));
      }
      return list;
    });
    const openSnapshotDetail=async(row)=>{
      snapshotDetailDialog.visible=true;
      snapshotDetailDialog.loading=true;
      snapshotDetailDialog.snapshot=null;
      snapshotDetailDialog.search='';
      snapshotDetailDialog.severityFilter='';
      try{
        const resp=await apiFetch(`${API_BASE}/api/v1/scan-compare/snapshots/${row.id}?with_issues=true`);
        if(resp.ok){
          snapshotDetailDialog.snapshot=await resp.json();
        }else{
          ElementPlus.ElMessage.error("获取快照明细失败");
        }
      }catch(e){
        ElementPlus.ElMessage.error("加载失败: "+e.message);
      }finally{
        snapshotDetailDialog.loading=false;
      }
    };
    const downloadSnapshotHtml=(row)=>{
      const t=authState.token;
      window.open(`${API_BASE}/api/v1/scan-compare/snapshots/${row.id}/html?access_token=${encodeURIComponent(t)}`,'_blank');
    };
    const deleteSnapshot=async(row)=>{
      try{
        await ElementPlus.ElMessageBox.confirm(`确定要删除 ${row.scan_label||('快照 #'+row.id)} 的扫描历史记录吗？删除后不可恢复。`,'确认删除',{
          type:'warning',
