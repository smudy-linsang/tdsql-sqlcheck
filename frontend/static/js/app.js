...
const app=createApp({
  setup(){
    ...
    const renderTrendChart=async()=>{const el=trendChartRef.value;if(!el)return;try{const resp=await apiFetch(`${API_BASE}/api/v1/dashboard/audit-trend?days=7`);const td=resp.ok?await resp.json():{dates:[],passed:[],failed:[]};const chart=echarts.init(el);chart.setOption({textStyle:{color:'#c3d3ec'},tooltip:{trigger:'axis'},legend:{data:['通过','拦截'],bottom:0,textStyle:{color:'#c3d3ec'}},grid:{left:'3%',right:'4%',bottom:'15%',top:'5%',containLabel:true},xAxis:{type:'category',data:td.dates||[],axisLabel:{color:'#8aa2c6'},axisLine:{lineStyle:{color:'#22345f'}}},yAxis:{type:'value',minInterval:1,axisLabel:{color:'#8aa2c6'},splitLine:{lineStyle:{color:'#22345f'}}},series:[{name:'通过',type:'bar',stack:'t',data:td.passed||[],itemStyle:{color:'#16a34a'}},{name:'拦截',type:'bar',stack:'t',data:td.failed||[],itemStyle:{color:'#dc2626'}}]})}catch(e){}};
    // P1-09: 审核项目选择用独立变量
