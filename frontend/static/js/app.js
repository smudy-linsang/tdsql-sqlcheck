  ...
  setup(){
    ...
    const deleteConn=async(row)=>{try{await ElementPlus.ElMessageBox.confirm(`确认删除连接「${row.name}」？`,'删除确认',{type:'warning'})}catch(e){return}try{await apiFetch(`${API_BASE}/api/v1/tdsql/connections/${row.id}`,{method:'DELETE'});ElementPlus.ElMessage.success('已删除');loadSavedConnections()}catch(e){ElementPlus.ElMessage.error('删除失败')}};
        ...
        const probeInstanceType=async(row)=>{try{const resp=await apiFetch(`${API_BASE}/api/v1/tdsql/connections/${row.id}/probe-instance-type`,{method:'POST'});const d=await resp.json();if(resp.ok){
          ...
          const html=`<div style="font-size:13px"><table style="border-collapse:collapse;width:100%;margin-bottom:10px"><tr style="background:#f8fafc"><th style="padding:4px 10px;border:1px solid #e5e7eb">判定源</th><th style="padding:4px 10px;border:1px solid #e5e7eb">可用</th><th style="padding:4px 10px;border:1px solid #e5e7eb">结论</th><th style="padding:4px 10px;border:1px solid #e5e7eb">说明</th></tr>${tr('管理员锁定',s.locked,s.locked&&s.locked.available?'终审，覆盖一切自动源':'未锁定')}${tr('ZK 管控面',s.zk,s.zk&&s.zk.kind?('形态 '+esc(s.zk.kind)):'')}${tr('SQL 探测',s.probe)}${tr('实例声明',s.declared,'来自实例配置')}</table><div style="padding:6px 10px;background:${d.conflict?'#fef3c7':'#ecfdf5'};border-radius:6px"><b>最终生效：${esc(instTypeCn(d.effective_instance_type))}</b>（来源：${esc(instSourceCn(d.instance_type_source))}）</div><div style="margin-top:8px;color:#475569">${esc(d.message||'')}</div></div>`;
