...
> **当前版本：v1.1.0** ｜ 更新日期：2026-07-14 ｜ 原厂诊断能力吸收(G1–G9)
> 面向商业银行生产环境的 TDSQL SQL 质量管控与慢 SQL 治理平台，纯内网部署、支持数百套 TDSQL 实例纳管。

## 核心能力

### 📝 SQL 审核（121 条规则 / 9 大分类）
...
## 文档体系（基线 v1.0.2 + v1.0.3.x 增量）

| 文档 | 说明 |
|---|---|
| [发布说明-v1.0.2](docs/发布说明-v1.0.2.md) | 版本定位、功能全景、已知限制（基线） |
| [部署手册-v1.0.2](docs/部署手册-v1.0.2.md) | 麒麟V10SP3 一键部署（**部署实施主文档**，基线） |
| [运维手册-v1.0.2](docs/运维手册-v1.0.2.md) | 启停/日志/监控/备份/故障/升级（基线） |
| [上线检查清单-v1.0.2](docs/上线检查清单-v1.0.2.md) | Go-Live 逐项检查（基线） |
...
| [V1.0.3 更新部署指南](docs/V1.0.3更新部署指南.md) | v1.0.3 升级与新增功能说明 |
| [V1.0.3.4 增量更新部署说明](docs/V1.0.3.4%E5%A2%9E%E9%87%8F%E6%9B%B4%E6%96%B0%E9%83%A8%E7%BD%B2%E8%AF%B4%E6%98%8E.md) | v1.0.3.4 补丁部署说明（含 1.0.3.1-1.0.3.3 历史记录） |
...
├── backend/                 # FastAPI 后端
│   ├── api/                 # 14 个路由模块 (/api/v1/*)
│   ├── engine/              # 审核引擎（sqlglot 解析器 + 121 条规则/9 分类）
...
└── docs/                    # v1.0.2 基线文档与 v1.0.3.x 增量部署说明
```
...
| 版本 | 日期 | 说明 |
|---|---|---|
| **v1.0.3.4** | 2026-07-13 | 缺陷修复：慢SQL多SET合并数据溢出（BUG-1）、慢SQL报告/看板严重度统计错误（BUG-2）（含 v1.0.3.1-v1.0.3.3 累积修复） |
| **v1.0.3** | 2026-07-10 | 新增上线检查模块（替代 tdsql_12.sh 脚本）、HTML报告导出、实例连接编辑、项目删除/停用分离、全局选择器移至页面内 |
